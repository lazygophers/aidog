...
void main() {
  ...
  group('TimeWindow.fromJson — 半时区脏数据归一（React: normalizeWindow）', () {
    test('整数 hour 原样', () {
      final w = TimeWindow.fromJson({'start_hour': 8, 'end_hour': 20});
      expect([w.startHour, w.startMinute, w.endHour], [8, 0, 20]);
    });
    test('8.5 拆成 8:30', () {
      final w = TimeWindow.fromJson({'start_hour': 8.5, 'end_hour': 20});
      expect([w.startHour, w.startMinute], [8, 30]);
    });
    test('已有 start_minute 时叠加进位（8.5 + 40 → 9:10）', () {
      final w = TimeWindow.fromJson({
        'start_hour': 8.5,
        'start_minute': 40,
        'end_hour': 20,
      });
    ...
    test('start/end 各自独立归一', () {
      final w = TimeWindow.fromJson({'start_hour': 8.5, 'end_hour': 20.25});
  ...
  group('PlatformCard（React: PlatformCard.test.tsx）', () {
    ...
    testWidgets('折叠态：渲染平台名 + 操作按钮；展开区默认收着', (tester) async {
      ...
      // 没接表单 → 不画编辑 / 复制按钮（点了没反应的按钮比没有更糟）。
      expect(find.text(c.t('platform.duplicate')), findsNothing);
