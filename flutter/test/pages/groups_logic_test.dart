...
void main() {
  ...
  group('模型映射：env_vars 必须透传，否则被后端 default 清空', () {
    test('添加映射时把当前 env_vars 一并带上（Groups.tsx:629-632 标的坑）', () async {
      final k = groupsFake(
        page: [
          {
            'group': {
      ...
      final input =
          k.lastCallTo('group_update')!.args!['input']! as Map<String, Object?>;
      expect(input['env_vars'], [
        {'key': 'KEEP', 'value': 'ME'},
      ]);
      expect((input['model_mappings']! as List).length, 1);
    });

    test('删映射时同样透传 env_vars', () async {
      final k = groupsFake(
        page: [
          {
            'group': {
            ...
            'platforms': <Object?>[],
            'model_mappings': [
              {'source_model': 'a', 'target_platform_id': 1, 'target_model': 'b'},
              {'source_model': 'c', 'target_platform_id': 1, 'target_model': 'd'},
            ],
          },
        ],
      );
      final c = GroupsController(invoke: k.fn);
      await c.load();
      await c.deleteMapping(10, 0);
      final input =
          k.lastCallTo('group_update')!.args!['input']! as Map<String, Object?>;
      expect(input['env_vars'], [
        {'key': 'KEEP', 'value': 'ME'},
      ]);
      final left = input['model_mappings']! as List;
      expect(left.length, 1);
      expect((left.first as Map)['source_model'], 'c');
    });

    test('四个字段缺一就不发命令（Groups.tsx:614 的早退）', () async {
      final k = groupsFake();
      final c = GroupsController(invoke: k.fn);
      await c.load();
      await c.addMapping(
