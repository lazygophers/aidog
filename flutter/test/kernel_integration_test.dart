...
void main() {
  group('RPC 往返（真实内核）', () {
    ...
    test('实测往返耗时：应当在 64 µs 量级，不是 862 µs（HttpClient 量级）', () async {
      ...
      //
      // 钉 **min** 而不是 p50：这条断言要区分的是「代码退化」与「机器当时很忙」。
      // 实测 2026-09-21 同一份代码，跑全量测试（同时在编 Rust）时 p50=4962µs、
      // 单独跑 p50=384µs —— p50 会被别的进程抢 CPU 直接推穿，连红两次都是误报。
      // 最快的那一次往返不会因为机器忙而变得更快，所以 min 只受代码影响：
      // 健康态实测 241µs；真退回 HttpClient 时连 min 也在毫秒量级。
      // 代价：只拖慢长尾、不动 min 的退化这条测不出来 —— 那类由上面的比值兜。
      expect(
        us.first,
        lessThan(1500),
        reason:
            'socket 最快一次 ${us.first}µs 已经离谱（p50=$p50µs，机器忙时 p50 会虚高，'
            '但 min 不会）——与走哪条路无关的整体退化',
      );
