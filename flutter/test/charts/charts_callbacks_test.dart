...
String tickText(SideTitles s, double v) => (s.getTitlesWidget(v, meta(s)) as Text).data!;
...
void main() {
  group('散点图回调', () {
    ...
    testWidgets('轴刻度走 formatters 的默认实现，tooltip 出三行', (tester) async {
      await tester.pumpWidget(host(AidogScatterChart(
        histogram: hist,
        xLabel: 'x',
        yLabel: 'y',
      ...
      final item = d.scatterTouchData.touchTooltipData
          .getTooltipItems(d.scatterSpots.first)!;
      // React: 延迟 · … / 成本 · … / n 次请求
      ...
      expect(
        d.scatterTouchData.touchTooltipData.getTooltipColor(d.scatterSpots.first),
        AidogColors.dark.surface2,
      );
    });
    ...
    testWidgets('触点不在点集里 → 抛，不静默显示 0', (tester) async {
      ...
      final d = tester.widget<ScatterChart>(find.byType(ScatterChart)).data;
      expect(
        () => d.scatterTouchData.touchTooltipData
            .getTooltipItems(ScatterSpot(999, 999)),
        throwsStateError,
      );
    });
  });
  ...
  group('堆叠面积回调', () {
    testWidgets('轴刻度 + tooltip 报各层自己的原值（不是累加值）', (tester) async {
      ...
      expect(tickText(d.titlesData.bottomTitles.sideTitles, d.minX), isNotEmpty);

      final spot = LineBarSpot(d.lineBarsData[0], 0, d.lineBarsData[0].spots[0]);
      final text = d.lineTouchData.touchTooltipData
          .getTooltipItems([spot]).single!
          .children!
          .map((c) => c.text)
          .join();
  ...
  group('折线图回调', () {
    testWidgets('触点指示器 + 右轴刻度反解回原值', (tester) async {
      ...
      expect(
        d.lineTouchData
            .getTouchedSpotIndicator(d.lineBarsData[0], const [0])
            .length,
        1,
      );
      // 左轴顶 → 反解回右轴域的顶（cost 原值量纲，不是 req 的量纲）
      final top = double.parse(tickText(d.titlesData.rightTitles.sideTitles, d.maxY));
    ...
    testWidgets('单序列时左轴刻度用那条序列的格式化', (tester) async {
      ...
      expect(tickText(lineData(tester).titlesData.leftTitles.sideTitles, 2), 'v2.0');
    });
  });

  testWidgets('空态副行显示出来', (tester) async {
    await tester.pumpWidget(host(const AidogLineChart(
      series: [],
  ...
  group('模型的相等性与重绘判定', () {
    ...
    test('TooltipRow', () {
      final a = tooltipRowAt([series('k', const [ChartPoint(0, 1)])], 0, 1);
      final b = tooltipRowAt([series('k', const [ChartPoint(0, 1)])], 0, 1);
      expect(a, b);
