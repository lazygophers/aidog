...
- **分组列表页的模型映射 UI 已删**（2026-09-23 用户拍板）：增删改只在分组编辑表单。
  订正（2026-09-25）：原先写的理由「React 列表态本来也没有映射 UI」是错的 ——
  React 列表态**有**整套（已存映射逐条只读展示 + 每行删除 + 「+ 添加映射」内联表单，
  `GroupListItem.tsx:477-553`）。所以这是**有意去掉查看入口**，不是补齐对齐：
  Flutter 侧列表态现在连「这个组有哪些映射」都看不到，要点进编辑才知道。
  裁决本身保留；要恢复只读展示的话，这是那条口子。
