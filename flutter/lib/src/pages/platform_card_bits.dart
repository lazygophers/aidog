...
// ══ 高峰时段（utils/timeWindow.ts）══════════════════════════════════

/// `defaults.ts:12::TimeWindow`。
class TimeWindow {
  const TimeWindow({
  ...
  factory TimeWindow.fromJson(Map<String, dynamic> j) {
    ...
    // —— `timeWindow.ts:46::normalizeWindow`。
    final (sh, sm) = _splitFraction(
      (j['start_hour'] as num?)?.toDouble() ?? 0,
      (j['start_minute'] as num?)?.toInt() ?? 0,
    );
    final (eh, em) = _splitFraction(
      (j['end_hour'] as num?)?.toDouble() ?? 0,
      (j['end_minute'] as num?)?.toInt() ?? 0,
    );
    return TimeWindow(
      startHour: sh,
      endHour: eh,
      ...
      daysOfWeek: (j['days_of_week'] as List?)
          ...
          .toList(),
      daysOfMonth: (j['days_of_month'] as List?)
          ?.map((e) => (e as num).toInt())
          .toList(),
      models: (j['models'] as List?)?.map((e) => '$e').toList(),
...
(int, int) _splitFraction(double h, int existingMinute) {
  if (h == h.roundToDouble()) return (h.toInt(), existingMinute);
  final hour = h.floor();
  final extra = ((h - hour) * 60).round();
  final m = (((hour * 60 + existingMinute + extra) % 1440) + 1440) % 1440;
  return (m ~/ 60, m % 60);
}

bool _tzInited = false;

/// `timeWindow.ts:68::wallTimeInTz`：按 IANA 时区取本地时刻分量。
/// 缺省 / 非法时区名回落 UTC（数据脏不炸 UI），与 Rust `peak::wall_time` 对称。
({int hour, int minute, int weekday, int dayOfMonth}) wallTimeInTz(
  ...
  // Dart 的 weekday 是 1=Mon…7=Sun，JS / Rust 那套是 0=Sun…6=Sat。
  return (
    hour: d.hour,
    minute: d.minute,
    weekday: d.weekday % 7,
...
/// `timeWindow.ts:163::modelMatch`：exact 或 `prefix*` 前缀通配。
...
bool _windowModelsHit(TimeWindow w, String requestModel) {
  if (requestModel.isEmpty) return true;
  final models = w.models;
  if (models == null || models.isEmpty) return true;
  return models.any((p) => modelMatch(p, requestModel));
}
...
bool _hit(
  ...
  final startMin = w.startHour * 60 + _clampMinute(w.startMinute);
  final endMin = w.endHour * 60 + _clampMinute(w.endMinute);
  final bool timeHit;
  if (endMin > startMin) {
  ...
  return _windowModelsHit(w, requestModel);
}

/// `timeWindow.ts:178::isCurrentlyPeak`：命中任一窗口 → true；空 / 无命中 → false。
