...
status: active
related: [sse-chunk-stateless-defect,hot-path-buffers]
---
...
- **一处实现上界常量** —— 如 `SSE_LINE_BUF_MAX_BYTES = 1MB` 或 `STREAM_BUF_CAP`
- **其余路径引用此常量** —— 禁硬编码、禁各写一份数值
...
[[sse-chunk-stateless-defect]] （流缓冲架构） · [[hot-path-buffers]] （mpsc 队列）
...
[[sse-chunk-stateless-defect]] 阐述流缓冲架构，[[hot-path-buffers]] 涉及 mpsc 队列背压。
