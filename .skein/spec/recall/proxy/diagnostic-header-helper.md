...
layer: recall
created: 1785514813
title: diagnostic-header-helper
category: proxy
keywords: [diagnostic-headers,request-tracing,debugging,headers]
status: active
inclusion: auto
---
layer: recall
...
何时被读: 改 `src-tauri/crates/aidog_core/src/gateway/proxy/` 的**响应构造点** / 新加诊断响应 header 时
谁读: main / sub-agent
...
grep -rn "inject_trace_header" src-tauri/crates/aidog_core/src/gateway/proxy/*.rs | grep -v test_ | wc -l

# 返路径遗漏检查 (每处返路径要么有 helper 调用, 要么 blind_relay 豁免注释)
grep -rn "\.into_response()\|Response::builder()\|ok_empty!" src-tauri/crates/aidog_core/src/gateway/proxy/*.rs | grep -v test_

# blind_relay 豁免注释存在
grep -n "blind_relay\|物理不可注入\|字节透传" src-tauri/crates/aidog_core/src/gateway/proxy/connect.rs
```
