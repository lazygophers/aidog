...
类目: arch(103), build(58), cross-layer(12), db(16), domain(74), ops(8), optimization(35), skein(24) · 关联见 [backlinks.md](backlinks.md)

| rule (topic.md#标题) | category | title | keywords | inclusion | anchors | status/出链 | summary |
|---|---|---|---|---|---|---|---|
| arch/adapter-deadcode-whitelist-authority.md#关联 | arch | 关联 | arch,rule,spec | auto | - | active / →rule-07 | [[rule-07]]  --- |
| arch/adapter-deadcode-whitelist-authority.md#案例 | arch | 案例 | arch,rule,spec | auto | - | active | - arch-deepen-2 commit `78e32df4`：删的 5 个 vendor adapter（glm_… |
| arch/adapter-deadcode-whitelist-authority.md#正解 | arch | 正解 | arch,rule,spec | auto | - | active | **唯一权威 = `gateway/proxy/forward.rs:85-86` 的 `is_valid_wire_p… |
| arch/adapter-deadcode-whitelist-authority.md#触发场景 | arch | 触发场景 | arch,rule,spec | auto | - | active | 删除 vendor adapter 文件或判定某 adapter 是否属于死代码时。 |
| arch/adapter-deadcode-whitelist-authority.md#适用 | arch | 适用 | arch,rule,spec | auto | - | active | - adapter 文件管理时 - protocol 数量变更 - 编码规范卡关：为什么要删这个文件 |
| arch/adapter-deadcode-whitelist-authority.md#陷阱 | arch | 陷阱 | arch,rule,spec | auto | - | active | 用文件名判定（如「vendor 名 = 协议名」），误删活代码；或遗漏实际有白名单的 adapter。 |
...
| arch/cross-db-subquery-handle-selection.md#Cross-ref | arch | Cross-ref | arch,rule,spec | auto | - | active / →auto-fix-downgrade-34 | - sqlite-cross-db-no-join（跨库禁 JOIN，强制拆闭包 + Rust 合并） - [[auto… |
| arch/cross-db-subquery-handle-selection.md#MUST 规则 | arch | MUST 规则 | arch,rule,spec | auto | - | active | 跨库补查闭包的 handle **必须按补查表的库归属选**，禁顺手复用主表 handle。 |
| arch/cross-db-subquery-handle-selection.md#正确写法（✅） | arch | 正确写法（✅） | arch,rule,spec | auto | - | active | ```rust // 主查走 log.db handle let logs = proxy_log_handle.cal… |
| arch/cross-db-subquery-handle-selection.md#错误样本（❌） | arch | 错误样本（❌） | arch,rule,spec | auto | - | active | ```rust // proxy_log 在 log.db，补查 cpp.name 在 platform.db prox… |
| arch/cross-db-subquery-handle-selection.md#验收 | arch | 验收 | arch,rule,spec | auto | - | active | ```bash # 找跨库补查点（同函数 / 同闭包内出现多库表名） grep -rn 'FROM "proxy_log… |
...
| arch/protocol-wire-str.md#关联 | arch | 关联 | arch,rule,spec | auto | - | active / →rule-05 | [[rule-05]]  --- |
| arch/protocol-wire-str.md#案例 | arch | 案例 | arch,rule,spec | auto | - | active | - gateway/models/protocol.rs:173 定义 wire_str() - arch-deepen… |
| arch/protocol-wire-str.md#正解 | arch | 正解 | arch,rule,spec | auto | - | active | 统一用 `Protocol::wire_str()` 方法序列化协议名。 |
| arch/protocol-wire-str.md#触发场景 | arch | 触发场景 | arch,rule,spec | auto | - | active | 在 proxy/forward 层需要获取协议名或序列化 Protocol enum 时。 |
| arch/protocol-wire-str.md#适用 | arch | 适用 | arch,rule,spec | auto | - | active | - Protocol enum 序列化时 - adapter 分发时协议名判定 |
| arch/protocol-wire-str.md#陷阱 | arch | 陷阱 | arch,rule,spec | auto | - | active | 禁手写 `serde_json::to_string(&x).trim_matches('"')` 或其他字符串转换，容… |
...
| build/cargo-workspace-hygiene.md#Cross-reference | build | Cross-reference | build,rule,spec | auto | - | active | - parent design：`.trellis/tasks/07-10-commands-restructure/d… |
| build/cargo-workspace-hygiene.md#GUI 冒烟降级（worktree 无 display 时） | build | GUI 冒烟降级（worktree 无 display 时） | build,rule,spec | auto | - | active | worktree 无 `node_modules` / 无 display 无法跑 `yarn tauri dev` 全… |
| build/cargo-workspace-hygiene.md#PoC 空骨架门禁 (MUST) | build | PoC 空骨架门禁 (MUST) | build,rule,spec | auto | - | active | 单 crate → workspace 多 crate 重构 **MUST 先建空骨架 PoC 门禁**，过才放行全量迁… |
| build/cargo-workspace-hygiene.md#PoC 门禁验收 (MUST，全量迁移前必过) | build | PoC 门禁验收 (MUST，全量迁移前必过) | build,rule,spec | auto | - | active | 1. `cargo build --workspace`：0 errors（含现 root crate + N 空壳 +… |
| build/cargo-workspace-hygiene.md#recall/cross-layer | build | recall/cross-layer | build,rule,spec | auto | - | active | - |
| build/cargo-workspace-hygiene.md#root 过渡路径迁移 (MUST) | build | root 过渡路径迁移 (MUST) | build,rule,spec | auto | - | active | core 提取后 root package **过渡保留**（binary crate C10 才建），加 `aidog… |
| build/cargo-workspace-hygiene.md#workspace.dependencies 版本对齐 (MUST) | build | workspace.dependencies 版本对齐 (MUST) | build,rule,spec | auto | - | active | - `[workspace.dependencies]` 版本号 + features **MUST 逐项照抄**现 r… |
| build/cargo-workspace-hygiene.md#子 crate 规范 (MUST) | build | 子 crate 规范 (MUST) | build,rule,spec | auto | - | active | - `name` 用下划线（`commands_platform` 等，非 hyphen；目录名连字符是 Cargo 惯… |
| build/cargo-workspace-hygiene.md#实例 | build | 实例 | build,rule,spec | auto | - | active | task 07-10-ws-skeleton（commands-restructure C1）：src-tauri 单 … |
| build/cargo-workspace-hygiene.md#核心提取下沉防循环范式 (MUST) | build | 核心提取下沉防循环范式 (MUST) | build,rule,spec | auto | - | active | PoC 空骨架过门后，业务代码入 `aidog_core` 时**MUST** 据依赖关系分类下沉，防 core→com… |
| build/cargo-workspace-hygiene.md#验收断言（可复用） | build | 验收断言（可复用） | build,rule,spec | auto | - | active | ```bash # baseline 不回归 cargo test --workspace --lib / grep -… |
| build/cargo-workspace-hygiene.md#验收断言（核心提取，可复用） | build | 验收断言（核心提取，可复用） | build,rule,spec | auto | - | active | ```bash # 路径迁移彻底（root 残留核心域路径 = 漏改） grep -rn 'crate::gateway… |
...
| build/shadcn-add-verify-deps.md#关联 | build | 关联 | build,rule,spec | auto | - | active / →shadcn-infra-31 | [[shadcn-infra-31]] (同任务产出的前端规则)  --- |
| build/shadcn-add-verify-deps.md#反例 | build | 反例 | build,rule,spec | auto | - | active | ❌ 只加 UI 组件不验证 cva → 运行时崩 ❌ 改 package.json 后不 yarn install → … |
| build/shadcn-add-verify-deps.md#案例 | build | 案例 | build,rule,spec | auto | - | active | - shadcn-infra task: 首次 `shadcn add` 后运行时崩，发现 cva 缺失 - 根因: y… |
| build/shadcn-add-verify-deps.md#触发场景 | build | 触发场景 | build,rule,spec | auto | - | active | 运行 `npx shadcn add` 批量添加组件后，依赖树中仅含 `@radix-ui/react-slot` 等 … |
| build/shadcn-add-verify-deps.md#适用 | build | 适用 | build,rule,spec | auto | - | active | yarn 4+ / pnp 环境，shadcn 批量 add 场景 |
| build/shadcn-add-verify-deps.md#陷阱-正解 | build | 陷阱-正解 | build,rule,spec | auto | - | active | - **陷阱**: shadcn CLI 在 yarn 4+ / pnp 环境下可能未正确解析 cva 传递依赖，只装直… |
...
| build/tauri-build-bundle.md#yarn tauri build --no-bundle 不产 .app | build | yarn tauri build --no-bundle 不产 .app | build,rule,spec | auto | - | active | - |
| build/tauri-build-bundle.md#反例（错误模式） | build | 反例（错误模式） | build,rule,spec | auto | - | active | / ❌ 错 / ✅ 改为 / /---/---/ / `yarn tauri build --no-bundle` / … |
| build/tauri-build-bundle.md#案例 | build | 案例 | build,rule,spec | auto | - | active | 性能测试中需要获取原始二进制做行为测试。尝试 `yarn tauri build --no-bundle` 后发现 `b… |
| build/tauri-build-bundle.md#触发场景 | build | 触发场景 | build,rule,spec | auto | - | active | Tauri macOS 构建时使用 `yarn tauri build --no-bundle` 时，只产生裸二进制 `… |
| build/tauri-build-bundle.md#适用 | build | 适用 | build,rule,spec | auto | - | active | - Tauri macOS 应用打包 - CI/CD 中需确保 .app 生成 - 区分二进制构建 vs app bun… |
| build/tauri-build-bundle.md#陷阱 & 正解 | build | 陷阱 & 正解 | build,rule,spec | auto | - | active | ❌ **陷阱**：假设 `--no-bundle` 仅跳过签名/通证，仍产 `.app`  ```bash yarn t… |
| build/vite-at-alias-manual.md#关联 | build | 关联 | build,rule,spec | auto | - | active / →shadcn-infra-28 | [[shadcn-infra-28]] (同任务 cva 依赖)  --- |
| build/vite-at-alias-manual.md#反例 | build | 反例 | build,rule,spec | auto | - | active | ❌ 只配 vite alias 不配 tsconfig → 类型检查报错 ❌ 用相对路径 `../../componen… |
| build/vite-at-alias-manual.md#案例 | build | 案例 | build,rule,spec | auto | - | active | - shadcn-infra task: shadcn 生成的组件含 `import @/components/xxx`… |
| build/vite-at-alias-manual.md#触发场景 | build | 触发场景 | build,rule,spec | auto | - | active | 使用 shadcn/ui 或其他假设存在 `@` 别名的库时，项目原无 `@` → `src` 的路径别名配置，导致 `… |
| build/vite-at-alias-manual.md#适用 | build | 适用 | build,rule,spec | auto | - | active | shadcn/ui 迁移、Vite 从零配置、路径别名标准化 |
| build/vite-at-alias-manual.md#陷阱-正解 | build | 陷阱-正解 | build,rule,spec | auto | - | active | - **陷阱**: shadcn 假设 vite 已有 `@` 别名（标准 scaffolding 如 Vite 默认模… |
...
| cross-layer/sole-platform-symmetry.md#单启用平台判定对称性 (Rust ↔ TS) | cross-layer | 单启用平台判定对称性 (Rust ↔ TS) | cross-layer,rule,spec | auto | - | active | - |
| cross-layer/sole-platform-symmetry.md#跨层对称硬规 (Rust ↔ TS) | cross-layer | 跨层对称硬规 (Rust ↔ TS) | cross-layer,rule,spec | auto | - | active | - |
| cross-layer/tauri-react-boundary-contract.md#CRUD Pattern (MUST) | cross-layer | CRUD Pattern (MUST) | cross-layer,rule,spec | auto | - | active | - 每个 resource 必须在 `api.ts` 提供 `{ create, list, get, update, … |
| cross-layer/tauri-react-boundary-contract.md#Data Flow (MUST) | cross-layer | Data Flow (MUST) | cross-layer,rule,spec | auto | - | active | - 数据流必须单向: Rust command → `invoke` → React `useState` → JSX … |
| cross-layer/tauri-react-boundary-contract.md#Format Contracts (MUST) | cross-layer | Format Contracts (MUST) | cross-layer,rule,spec | auto | - | active | - 后端返回 timestamp 必须为 ISO 8601 string (`chrono::DateTime<Utc>… |
| cross-layer/tauri-react-boundary-contract.md#Rust enum → type alias arbitrary 全 JSON 驱动 (MUST) | cross-layer | Rust enum → type alias arbitrary 全 JSON 驱动 (MUST) | cross-layer,rule,spec | auto | - | active | Rust enum 当变体集合属「后端 JSON 真值源派生」类（值集合由 `src-tauri/defaults/*.… |
| cross-layer/tauri-react-boundary-contract.md#Rust 执行层 match 臂 → JSON 真值源配置驱动引擎 (MUST) | cross-layer | Rust 执行层 match 臂 → JSON 真值源配置驱动引擎 (MUST) | cross-layer,rule,spec | auto | - | active | Rust 执行层（如 proxy headers 注入）写死 per-variant dispatch (`match … |
| cross-layer/tauri-react-boundary-contract.md#Tauri 窗口生命周期事件 (MUST) | cross-layer | Tauri 窗口生命周期事件 (MUST) | cross-layer,rule,spec | auto | - | active | - 窗口生命周期事件 (失焦 `Focused` / 关闭 `CloseRequested` / 缩放 `Resized… |
| cross-layer/tauri-react-boundary-contract.md#Tauri↔React Boundary (MUST) | cross-layer | Tauri↔React Boundary (MUST) | cross-layer,rule,spec | auto | - | active | - 后端新增 Tauri command 必须在前端 `api.ts` 添加对应 invoke 包装函数 - invok… |
| cross-layer/tauri-react-boundary-contract.md#Verification | cross-layer | Verification | cross-layer,rule,spec | auto | - | active | ```bash # 所有 invoke 集中在 api.ts grep -rn 'invoke(' src/ / gre… |
| cross-layer/tauri-react-boundary-contract.md#反模式 (禁) | cross-layer | 反模式 (禁) | cross-layer,rule,spec | auto | - | active | / 反模式 / 正确做法 / 触发后果 / / --- / --- / --- / / `invoke(` 散落在组件 … |
| cross-layer/tauri-react-boundary-contract.md#持久化路径换、公共契约零改 (MUST) | cross-layer | 持久化路径换、公共契约零改 (MUST) | cross-layer,rule,spec | auto | - | active | 换持久化路径（专属表 → `setting` / JSON / 他处）时，跨 Rust↔TS **公共契约层禁改** —… |
| db/db-table-conventions.md#Column Naming (MUST) | db | Column Naming (MUST) | db,rule,spec | auto | - | active | - 平台主类型列名为 `platform_type`（禁 `protocol`）；其值用 `serde_json::to… |
| db/db-table-conventions.md#Migration (MUST) | db | Migration (MUST) | db,rule,spec | auto | - | active | - schema 破坏式变更必须提供独立一次性迁移脚本（`scripts/`，非 app 运行时代码），迁移完成后删除 … |
| db/db-table-conventions.md#No NULL (MUST) | db | No NULL (MUST) | db,rule,spec | auto | - | active | - 所有 `TEXT` 列 `NOT NULL DEFAULT ''`；所有 `INTEGER` 列 `NOT NULL… |
| db/db-table-conventions.md#Primary Key (MUST) | db | Primary Key (MUST) | db,rule,spec | auto | - | active | - 业务表主键必须 `id INTEGER PRIMARY KEY AUTOINCREMENT`，Rust 映射 `u6… |
| db/db-table-conventions.md#Relations & Mappings (MUST) | db | Relations & Mappings (MUST) | db,rule,spec | auto | - | active | - 关联表（如 `group_platform`）加代理 `id` 自增主键 + 保留业务复合 `UNIQUE(grou… |
| db/db-table-conventions.md#Soft Delete (MUST) | db | Soft Delete (MUST) | db,rule,spec | auto | - | active | - 删除必须逻辑删：`UPDATE <table> SET deleted_at = <now_ms> WHERE id… |
| db/db-table-conventions.md#Table Naming (MUST) | db | Table Naming (MUST) | db,rule,spec | auto | - | active | - 表名必须**单数**，禁复数：`platform` / `group` / `group_platform` / `… |
| db/db-table-conventions.md#Time Fields (MUST) | db | Time Fields (MUST) | db,rule,spec | auto | - | active | - 每个表必须含 `created_at` / `updated_at` / `deleted_at`，类型 `INTE… |
| db/db-table-conventions.md#Verification | db | Verification | db,rule,spec | auto | - | active | ```bash # 复数表名残留 sqlite3 ~/.aidog/aidog.db ".tables" / grep … |
| db/db-table-conventions.md#recall/domain | db | recall/domain | db,rule,spec | auto | - | active | - |
| db/db-table-conventions.md#专属表 → setting 迁移模式 (MUST) | db | 专属表 → setting 迁移模式 (MUST) | db,rule,spec | auto | - | active | 域数据从专属表迁通用 `setting` 表时（`scope=<域>, key=<实体>` JSON），走 app 内置… |
| db/filter-semantics.md#排斥列默认过滤需明确确认为产品设计意图 | db | 排斥列默认过滤需明确确认为产品设计意图 | db,rule,spec | auto | - | active | 当 task 涉及「默认排斥某类请求」的过滤逻辑时（如 Logs 主页默认隐藏 test/quota 请求），确认这是*… |
| db/pagination-offset.md#LIMIT+1 探测分页无精确总数 | db | LIMIT+1 探测分页无精确总数 | db,rule,spec | auto | - | active | 当分页 UI 仅需「有无下一页」而不需精确总数时，改用 LIMIT offset+pageSize+1 探测有下一页，而… |
...
| db/sqlite-partial-index.md#参数化查询无法触发 partial index（字面量盲区） | db | 参数化查询无法触发 partial index（字面量盲区） | db,rule,spec | auto | - | active | SQLite 查询规划器对 partial index 的匹配仅识别 SQL 文本中的**字面量常量**谓词，不识别**… |
...
| domain/claude-code-passthrough-platform.md#Frontend (MUST) | domain | Frontend (MUST) | domain,rule,spec | auto | - | active | - `api.ts` Protocol union 含 `/ "claude_code"` - `Platforms.t… |
| domain/claude-code-passthrough-platform.md#Intercept Point (MUST) | domain | Intercept Point (MUST) | domain,rule,spec | auto | - | active | - 拦截点：`select_platform` 之后、`convert_request` 之前（与 mock 拦截点同区… |
| domain/claude-code-passthrough-platform.md#No Transform / No Inject (MUST) | domain | No Transform / No Inject (MUST) | domain,rule,spec | auto | - | active | - 禁 `convert_request` / 禁 `build_upstream_headers` / 禁 `appl… |
| domain/claude-code-passthrough-platform.md#Original Request Capture (MUST) | domain | Original Request Capture (MUST) | domain,rule,spec | auto | - | active | - `proxy.rs` handle_proxy 在 `req.into_parts()` **之前**捕获原始量（对… |
| domain/claude-code-passthrough-platform.md#Verification | domain | Verification | domain,rule,spec | auto | - | active | ```bash cd src-tauri && cargo test passthrough   # URL 拼接 / … |
| domain/claude-code-passthrough-platform.md#What & When (MUST) | domain | What & When (MUST) | domain,rule,spec | auto | - | active | - `Protocol::ClaudeCode`（`models.rs`，serde rename `"claude_c… |
| domain/claude-code-passthrough-platform.md#handle_passthrough Semantics (MUST) | domain | handle_passthrough Semantics (MUST) | domain,rule,spec | auto | - | active | 1. **目标 URL** = `base_url` + 客户端原始 path（+ query）。**约定 CC 平台 … |
| domain/claude-code-passthrough-platform.md#proxy_log (MUST) | domain | proxy_log (MUST) | domain,rule,spec | auto | - | active | - 透传分支**正常记** `proxy_log`：   - `source_protocol` = `target_p… |
| domain/coding-plan-no-public-quota-api.md#coding-plan-no-public-quota-api | domain | coding-plan-no-public-quota-api | domain,rule,spec | auto | - | active | bailian/qianfan/xiaomi/compshare 等 coding plan 订阅制平台上游均无公开程序… |
| domain/converter-normalized-intermediate.md#关联 | domain | 关联 | converter NonStreamResponse parse render protocol | auto | - | active / →rule-52,rule-54 | [[rule-52]] [[rule-54]] |
...
| domain/cpa-oauth-credential-format.md#Cross-ref | domain | Cross-ref | domain,rule,spec | auto | - | active / →auto-fix-downgrade-35,parser-multi-path-format-symmetry | - `src-tauri/crates/aidog_core/src/gateway/cpa_import/parser… |
| domain/cpa-oauth-credential-format.md#OAuth 类型枚举（CpaOAuthType） | domain | OAuth 类型枚举（CpaOAuthType） | domain,rule,spec | auto | - | active | codex / claude / kimi / xai / vertex / aistudio / antigravit… |
| domain/cpa-oauth-credential-format.md#多账号语义（CLIProxyAPI） | domain | 多账号语义（CLIProxyAPI） | domain,rule,spec | auto | - | active / →auto-fix-downgrade-35 | - 同一 OAuth 类型(如 xai)可有多个凭据(各 email 不同)→ **各自独立平台**(负载均衡) - d… |
| domain/cpa-oauth-credential-format.md#格式结构 | domain | 格式结构 | domain,rule,spec | auto | - | active | CLIProxyAPI OAuth 凭据 JSON(auth-dir 文件 / 导出 zip 内): ```json {… |
| domain/cpa-oauth-credential-format.md#识别逻辑 | domain | 识别逻辑 | domain,rule,spec | auto | - | active | - `parse_oauth_json(content) -> Option<Vec<CpaProvider>>`(pa… |
...
| domain/endpoint-cross-protocol-fallback.md#案例 | domain | 案例 | - | auto | - | active / →rule-06,rule-07 | - endpoint-cross-protocol-fallback task：普通平台步骤 4 泛化（同协议 > op… |
...
| domain/five-wire-protocols-anchor.md#关联 | domain | 关联 | protocol endpoint converter platform_type | auto | - | active / →rule-05,rule-53 | [[rule-05]] [[rule-53]] |
...
| domain/mock-platform-contract.md#Config Carrier — extra.mock (MUST) | domain | Config Carrier — extra.mock (MUST) | domain,rule,spec | auto | - | active | - mock 配置载体必须为现有 `platform.extra`（TEXT JSON 列），禁新增专用 DB 列（零迁… |
| domain/mock-platform-contract.md#Response Builders (MUST) | domain | Response Builders (MUST) | domain,rule,spec | auto | - | active | - 非流式: `build_response(cfg, source_protocol, model)` 按 5 协议返… |
| domain/mock-platform-contract.md#Three-Layer Config Override (MUST) | domain | Three-Layer Config Override (MUST) | domain,rule,spec | auto | - | active | 最终生效值 = 逐字段按优先级取首个存在者（`resolve_mock_config(extra, chat_req, … |
| domain/mock-platform-contract.md#Verification | domain | Verification | domain,rule,spec | auto | - | active | ```bash cd src-tauri && cargo test mock   # 全部通过（三层覆盖 / 5 协议… |
| domain/mock-platform-contract.md#What & When (MUST) | domain | What & When (MUST) | domain,rule,spec | auto | - | active | - `Protocol::Mock`（`models.rs`，serde rename `"mock"`）是**平台主类… |
| domain/mock-platform-contract.md#error_mode Semantics (MUST) | domain | error_mode Semantics (MUST) | domain,rule,spec | auto | - | active | `handle_mock`（proxy.rs）按 `error_mode` 分派，两类语义并存（delay 与 erro… |
| domain/mock-platform-contract.md#proxy_log (MUST) | domain | proxy_log (MUST) | domain,rule,spec | auto | - | active | - mock 分支直接写最终生效值 `log.{input_tokens,output_tokens,cache_tok… |
...
| domain/protocol-logo-fallback-chain.md#HTTP client (MUST) | domain | HTTP client (MUST) | domain,rule,spec | auto | - | active | - **MUST 复用 `build_http_client_system`** (非 `build_http_clie… |
| domain/protocol-logo-fallback-chain.md#presets JSON 读取 (MUST) | domain | presets JSON 读取 (MUST) | domain,rule,spec | auto | - | active | - `read_local_presets_json` 优先级: `~/.aidog/platform-presets.… |
| domain/protocol-logo-fallback-chain.md#三路 fallback 顺序 (MUST, 首成功即止) | domain | 三路 fallback 顺序 (MUST, 首成功即止) | domain,rule,spec | auto | - | active | 固定顺序, **禁重排**, 见 `sync_one_into`:  1. **simpleicons CDN** — … |
| domain/protocol-logo-fallback-chain.md#入口 | domain | 入口 | domain,rule,spec | auto | - | active | - `sync_all_logos(db, app_data_dir)` — 后台批量同步 (app 启动 / 手动触发… |
| domain/protocol-logo-fallback-chain.md#关联 | domain | 关联 | domain,rule,spec | auto | - | active | - [http-client-forward.md](./http-client-forward.md) — build… |
| domain/protocol-logo-fallback-chain.md#缓存契约 (MUST) | domain | 缓存契约 (MUST) | domain,rule,spec | auto | - | active | - 缓存路径 `~/.aidog/logos/<protocol_id>.png` (`logo_cache_path`… |
| domain/protocol-logo-fallback-chain.md#验收基准 (可复用) | domain | 验收基准 (可复用) | domain,rule,spec | auto | - | active | - [ ] 清空 `~/.aidog/logos/` 后, 有 `logo_url` 的 protocol 命中路 1;… |
| domain/protocol-logo-fallback-chain.md#验证命令 | domain | 验证命令 | domain,rule,spec | auto | - | active | ```bash # 三路 URL 模板存在且顺序 grep -n "cdn.simpleicons.org\//favi… |
| domain/reasoning-content-as-text-block.md#关联 | domain | 关联 | reasoning thinking anthropic signature converter | auto | - | active / →rule-52,rule-53 | [[rule-53]] [[rule-52]] |
