...
## 验收标准
- [ ] 归因结论落盘 findings.md，并明确推翻或证实 task 登记时的「retention 疑失效」假设
- [ ] log.db 主库体积降到 1GB 以下
- [ ] WAL 已 checkpoint 归零，PRAGMA journal_mode 仍为 wal（未被 VACUUM 改坏）
- [ ] 清理前后数据（体积/行数/journal_mode）已落盘为可复核的执行记录
- [ ] 清理用的临时脚本已删除，aidog 应用已恢复运行
- [ ] 删行带来的历史统计丢失已如实向用户说明（不默默换策略）
## 索引
