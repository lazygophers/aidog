...
大 log.db 是「cache 必被填满」的**充分条件**。库小则 cache 本来就填不满，本优化对新用户零收益。故：

- 每次采样记录 log.db 与 WAL 实际体积，大库组要求 **≥500MB**（用户 2026-07-31 拍板下调，原为 ≥5GB）
- 另跑一组 <100MB 小库对照，回答「定值对小库用户是否安全」

**门槛下调依据**（原 ≥5GB 的理由是「保证复现性」，非机理必需）：真实库在 logdb-size-governance
清理后降至 736MB。机理只需「库体积 >> cache 上限 27×2MB = 54MB」，736MB 是 13.6 倍，page cache
照样填满 —— `probe` 已实测坐实（default 9006 块 vs 64KB 档 1895 块，降 79%）。

**已知代价**：小库下三条查询 p95 是微秒级，run-to-run 噪声大（harness 已如实标注）。
baseline/sweep 需加大样本量（n 从 30 上调）压噪声；若加样本仍压不住，p95 那条验收按实测噪声
区间放宽并记录，禁在噪声里硬读趋势。
