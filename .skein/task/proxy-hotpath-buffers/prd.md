...
## 验收标准
- [ ] 存在一条就红的复现用例证明跨 chunk 多字节 UTF-8 字符当前会变 U+FFFD，修复后该用例转绿
- [ ] 50 路 mock 并发长流下，upstream_body / client_body 的累积字节受 STREAM_BODY_MAX_BYTES 约束，超限置截断标记而非继续 push
- [ ] 50 路 mock 并发压测的进程 phys_footprint 峰值相对基线下降，且给出下降归因
- [ ] upsert_log 的 clone 只发生在 terminal 分支或队列有容量时
- [ ] 请求 body 深拷贝次数从最多 3 次降到 1 次以内，用计数或 trace 证明
- [ ] get_group_platforms 命中缓存时不产生 SQL 查询，且现有失效触发点覆盖所有写入路径
- [ ] tracing 文件写不再发生在 tokio worker 线程上
- [ ] sse_line_buf 有明确上界，触发上界时行为已定义且不 panic
- [ ] 转发延迟 p95 与首 token 时延 p95 相对基线不上升（50 路 mock 并发）
- [ ] token 计数与 est_cost 在改动前后对同一组 mock 请求逐条一致
