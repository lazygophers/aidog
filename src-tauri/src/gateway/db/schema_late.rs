...
"#,
                ...
                // Migration 042: mitm_whitelist 加 rule_type 列（Clash 规则集 4 类型支持）。
                // rule_type: domain / suffix / keyword / ipcidr；host_pattern 复用列存规则值
                // （语义泛化，不改列名避迁移成本）。
                // 回填存量：`*.domain` → suffix（去 *. 前缀，Clash DOMAIN-SUFFIX 标准含裸域），
                // 无 *. 前缀 → domain（旧精确 host）。新库默认 DEFAULT 'suffix'（Clash 规则集多数）。
                let has_mitm_whitelist_rule_type = conn
                    .prepare("PRAGMA table_info(mitm_whitelist)")?
                    .query_map([], |r| r.get::<_, String>(1))?
                    .filter_map(Result::ok)
                    .any(|c| c == "rule_type");
                if !has_mitm_whitelist_rule_type {
                    let _ = conn.execute(
                        "ALTER TABLE mitm_whitelist ADD COLUMN rule_type TEXT NOT NULL DEFAULT 'suffix'",
                        [],
                    );
                    // 回填：`*.domain` → suffix + 同步把 host_pattern 去 *. 前缀（substr 3 跳过 `*.` 两字符）
                    // （Clash DOMAIN-SUFFIX 语义已是裸域，去 * 后命中面扩到含裸域，不破行为）
                    let _ = conn.execute(
                        "UPDATE mitm_whitelist SET rule_type='suffix', host_pattern=substr(host_pattern, 3) WHERE host_pattern LIKE '*.%'",
                        [],
                    );
                    // 无 *. 前缀且无点号的存量精确 host → domain（如纯 hostname）
                    let _ = conn.execute(
                        "UPDATE mitm_whitelist SET rule_type='domain' WHERE rule_type='suffix' AND host_pattern NOT LIKE '%.%'",
                        [],
                    );
                    // 注意：无 *. 前缀但含点的存量 host（如已配平台 base_url 提取的 api.anthropic.com）
                    // 保持 DEFAULT 'suffix' —— 现状 *.anthropic.com 只匹配子域、不匹配裸域，
                    // 回填去 * 后语义扩展为「裸域+子域」，对存量 default 已配平台 host 更宽松不破坏行为
                    // （MITM 命中面只会变大，不会缩，防回填后某些 host 失配降级 blind_relay）。
                }
...
/// host 来源：
///  - 固定 Clash 规则集（DEFAULT_RULES，37 条：Claude 3 + OpenAI 34，含 domain/suffix/keyword/ipcidr）：
///    覆盖官方 API 域族（api.anthropic.com / claude.ai / api.openai.com / chatgpt.com 等），
///    来源 blackmatrix7/ios_rule_script Clash 规则集。
...
fn seed_default_whitelist_if_empty(conn: &Connection) {
    ...
    // 已配平台 base_url host（domain 精确 host，非通配）。仅未删除平台。
    ...
    for base_url in hosts {
        if let Some(host) = crate::gateway::proxy::endpoint_host(&base_url) {
            let _ = conn.execute(
                "INSERT OR IGNORE INTO mitm_whitelist (host_pattern, rule_type, enabled, source, created_at) VALUES (?1, 'domain', 1, 'default', ?2)",
                params![host, now],
            );
        }
    }
...
mod tests {
    ...
    fn migrations_late_mitm_tables_and_default_whitelist_040_041() {
        ...
        // ② 默认 Clash 规则集 seed（37 条 = Claude 3 + OpenAI 34，含 domain/suffix/keyword/ipcidr）
        //   + 已配平台 host（api.anthropic.com，domain 类型）
        ...
        assert!(
            default_count >= 37,
            "default whitelist should contain the 37 Clash ruleset entries, got {}",
            default_count
        );

        // ③ 已配平台 base_url host 被提取（api.anthropic.com，rule_type=domain）
        let has_platform_host: bool = conn
            .query_row(
                "SELECT COUNT(*) FROM mitm_whitelist WHERE host_pattern = 'api.anthropic.com' AND rule_type = 'domain'",
                [],
                |r| r.get::<_, i64>(0),
            )
            .unwrap()
        ...
        // ④ suffix 规则存在（anthropic.com，Clash DOMAIN-SUFFIX 标准含裸域+子域）
        let has_suffix_anthropic: bool = conn
            .query_row(
                "SELECT COUNT(*) FROM mitm_whitelist WHERE host_pattern = 'anthropic.com' AND rule_type = 'suffix'",
                [],
                |r| r.get::<_, i64>(0),
            )
            .unwrap()
            > 0;
        assert!(has_suffix_anthropic, "anthropic.com (suffix) should be in default whitelist");
        ...
        for (rule_type, expected) in [
            ...
            let n: i64 = conn
                .query_row(
                    "SELECT COUNT(*) FROM mitm_whitelist WHERE rule_type = ?1 AND host_pattern = ?2",
                    params![rule_type, expected],
                    |r| r.get(0),
                )
                .unwrap();
            assert!(n > 0, "default whitelist should contain rule_type={rule_type} host_pattern={expected}");
        }
    }

    /// Migration 042 验收：rule_type 列回填存量（旧 `*.domain` → suffix + 去 *．前缀；
    /// 旧无 *．前缀 → domain）。模拟 041 已 seed 完后，042 加列回填的正确性。
    #[test]
    fn migrations_late_042_rule_type_backfill_legacy_wildcard() {
        let conn = Connection::open_in_memory().unwrap();
        // 模拟旧 schema（无 rule_type 列）+ 旧 seed 数据（*.anthropic.com / *.openai.com / api.deepseek.com）
        conn.execute_batch(r#"
            CREATE TABLE "group" (
                id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL DEFAULT '',
                group_key TEXT NOT NULL DEFAULT '', routing_mode TEXT NOT NULL DEFAULT '',
            ...
            CREATE TABLE proxy_log (id TEXT PRIMARY KEY, group_key TEXT, platform_id INTEGER, model TEXT, actual_model TEXT, source_protocol TEXT, target_protocol TEXT, status_code INTEGER, duration_ms INTEGER, input_tokens INTEGER, output_tokens INTEGER, cache_tokens INTEGER, est_cost REAL, is_stream INTEGER, retry_count INTEGER, blocked_by TEXT, blocked_reason TEXT, request_url TEXT, request_headers TEXT, request_body TEXT, upstream_request_url TEXT, upstream_request_headers TEXT, upstream_request_body TEXT, upstream_status_code INTEGER, upstream_response_headers TEXT, user_response_headers TEXT, user_response_body TEXT, response_body TEXT, created_at INTEGER, updated_at INTEGER, deleted_at INTEGER NOT NULL DEFAULT 0, attempts TEXT);
            CREATE TABLE IF NOT EXISTS settings (scope TEXT, key TEXT, value TEXT, PRIMARY KEY (scope, key));
            CREATE TABLE IF NOT EXISTS group_platform (id INTEGER PRIMARY KEY, group_id, platform_id, priority, weight INTEGER, level_priority INTEGER NOT NULL DEFAULT 5);
            CREATE TABLE IF NOT EXISTS notification (id TEXT PRIMARY KEY, created_at INTEGER);
            -- 旧 mitm_whitelist 表（无 rule_type 列），含 041 旧 seed 三种形态
            ...
            INSERT INTO mitm_whitelist (host_pattern, source, created_at) VALUES
                ('*.anthropic.com', 'default', 100),
                ('*.openai.com', 'default', 101),
                ('api.deepseek.com', 'default', 102);
        "#).unwrap();
        ...
        let has_rule_type = conn
            ...
            .filter_map(Result::ok)
            .any(|c| c == "rule_type");
        assert!(has_rule_type, "rule_type column should be added");

        // ② 存量 *.domain 回填为 suffix + 去 *．前缀（host_pattern=anthropic.com/openai.com）
        let suffix_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM mitm_whitelist WHERE rule_type='suffix' AND host_pattern IN ('anthropic.com', 'openai.com')",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(suffix_count, 2, "*.anthropic.com / *.openai.com should backfill to suffix + de-*");

        // ③ 存量精确 host（无点号边界后）保留为 suffix DEFAULT（含 api.deepseek.com，去 *．前后语义不破）
        let deepseek: (String, String) = conn
            .query_row(
                "SELECT host_pattern, rule_type FROM mitm_whitelist WHERE host_pattern LIKE '%deepseek%'",
                [],
                |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)),
            )
            .unwrap();
        assert_eq!(deepseek.0, "api.deepseek.com"); // 未被改
        // 注意：DEFAULT 'suffix' 应用到含点号的存量 host（api.deepseek.com），不破行为
        // （MITM 命中面只会变大：旧 *.deepseek.com 仅子域，新 suffix 含裸域）
        assert_eq!(deepseek.1, "suffix");

        // ④ 新 seed 不触发（表非空，count>0 跳过）
        let total: i64 = conn
            .query_row("SELECT COUNT(*) FROM mitm_whitelist", [], |r| r.get(0))
            .unwrap();
        assert_eq!(total, 3, "table non-empty → new 37-rule seed skipped");
    }
}