...
//! （test_schema.rs 自 eedd2943 拆 crate 起为孤儿模块未编译，勿再往里加测试。）
    ...
    fn cleanup_delisted_platform_rows_soft_deletes_unconfigured_only() {
        ...
        let empty_id: i64 = conn
            .query_row("SELECT id FROM platform WHERE name = 'micu-empty'", [], |r| r.get(0))
            .unwrap();
        conn.execute(
        ...
        let soft_deleted: i64 = conn
            .query_row("SELECT deleted_at FROM platform WHERE name = 'micu-empty'", [], |r| r.get(0))
            .unwrap();
        assert!(soft_deleted > 0, "未配置的下架行应被软删");
        let kept: i64 = conn
            .query_row("SELECT deleted_at FROM platform WHERE name = 'dmxapi-in-use'", [], |r| r.get(0))
            .unwrap();
        assert_eq!(kept, 0, "填过 key 的下架行必须保留");
        let untouched: i64 = conn
            .query_row("SELECT deleted_at FROM platform WHERE name = 'pipellm'", [], |r| r.get(0))
            .unwrap();
        assert_eq!(untouched, 0, "非清单平台不受影响");
        let memberships: i64 = conn
            .query_row("SELECT COUNT(*) FROM group_platform WHERE platform_id = ?1", [empty_id], |r| r.get(0))
            .unwrap();
    ...
    fn cleanup_delisted_registry_mirror_rows_removes_only_delisted_codes() {
        ...
        for code in ["micu", "packycode", "pipellm"] {
            conn.execute("INSERT INTO platform_preset (code) VALUES (?1)", [code]).unwrap();
            conn.execute("INSERT INTO model_entry (platform_code) VALUES (?1)", [code]).unwrap();
        }
        ...
        let presets: Vec<String> = conn
            ...
            .collect::<Result<_, _>>()
            .unwrap();
        assert_eq!(presets, vec!["pipellm".to_string()], "镜像表只留下架清单外的 code");
