use crate::helpers::*;
use crate::models::*;
use crate::{STATS_AGG_HOURLY_SQL, load_auto_from_map, now};
...
pub fn run_migrations_late(
    ...
    tracing::info!(
        "migration 20260727-14 (原 035): dropped redundant indexes (proxy_log/stats_agg部分在proxy_log_late)"
    );
    ...
    if main_count == 0 {
        ...
        let is_memory =
            main_file.is_empty() || main_file == ":memory:" || main_file.contains("mode=memory");
        if !is_memory {
            ...
            if log_path.exists() {
                ...
                let migrate_result = (|| -> SqlResult<()> {
                    conn.execute_batch(&format!("ATTACH DATABASE '{log_path_sql}' AS src_log"))?;
                    ...
                    if has_table {
                        ...
                        tracing::info!(
                            "migration 20260727-17 (原 052): stats_agg_hourly 跨库搬迁 log.db → main 完成"
                        );
                    }
...
fn translate_legacy_middleware_rules(conn: &Connection) -> SqlResult<usize> {
    ...
    for (id, rule_type, scope, scope_ref, match_type, pattern, action, config) in rows {
        ...
        let effective_pattern =
            if pattern.is_empty() && matches!(rule_type.as_str(), "content_filter" | "redaction") {
                None
            } else {
                Some(pattern.clone())
            };
        let leaf = |target: &str, pat: &str| json!({ "kind": "leaf", "target": target, "field": "", "match_type": match_type, "pattern": pat });
        let conditions: Option<serde_json::Value> = match (&target, &effective_pattern) {
            (Some(t), Some(p)) if p.is_empty() && rule_type == "error_rule" => {
                ...
                Some(
                    json!({ "kind": "leaf", "target": "status", "field": "", "match_type": "regex", "pattern": "[0-9]+" }),
                )
            }
...
pub fn run_migrations_proxy_log_late(
    ...
    for pid in cpa_pids {
        let _ = conn.execute("DELETE FROM proxy_log WHERE platform_id = ?1", params![pid]);
        let _ = conn.execute(
...
pub fn run_migrations_platform_late(conn: &Connection) -> SqlResult<()> {
    ...
    if let Ok(mut stmt) =
        conn.prepare("SELECT id, endpoints FROM platform WHERE platform_type = 'kimi'")
    {
        ...
        for (id, endpoints_json) in rows {
            ...
            if changed {
                ...
                tracing::info!(
                    platform_id = id,
                    "migration 20260727-01 (原 012): kimi coding endpoint client_type codex_tui→claude_code"
                );
            }
        }
    }
    ...
    let _ = conn.execute(
        "ALTER TABLE platform ADD COLUMN auto_group INTEGER NOT NULL DEFAULT 1",
        [],
    );
...
"#,
        ...
        conn.prepare("SELECT id, endpoints FROM platform WHERE platform_type = 'glm'")
    {
        ...
        for (id, endpoints_json) in rows {
            ...
            if changed {
                ...
                tracing::info!(
                    platform_id = id,
                    "migration 20260727-05 (原 025): glm coding-plan anthropic endpoint coding_plan→true"
                );
            }
        }
    }
    ...
    if has_breaker_col {
        ...
        let _ = conn.execute(
            "ALTER TABLE platform DROP COLUMN breaker_failure_threshold",
            [],
        );
        ...
        tracing::info!(
            "migration 20260727-06 (原 026): backfilled breaker into extra + dropped auto_group/breaker_* columns"
        );
    }

    // Migration 20260727-07 (原 027): 默认分组标记（已在 platform_early 建表含此列，ALTER 幂等）。
    let _ = conn.execute(
        "ALTER TABLE \"group\" ADD COLUMN is_default INTEGER NOT NULL DEFAULT 0",
        [],
    );

    // Migration 20260727-08 (原 029): group_platform level_priority（已在 platform_early 建表含此列，ALTER 幂等）。
    let _ = conn.execute(
        "ALTER TABLE group_platform ADD COLUMN level_priority INTEGER NOT NULL DEFAULT 5",
        [],
    );
...
fn strip_w2_peak_hours_copies(conn: &Connection) {
    let rows: Vec<(i64, String)> =
        match conn.prepare("SELECT id, extra FROM platform WHERE extra LIKE '%peak_hours%'") {
            Ok(mut stmt) => stmt
                .query_map([], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?)))
                .ok()
                .map(|iter| iter.filter_map(Result::ok).collect())
    ...
    if cleaned > 0 {
        tracing::info!(
            cleaned,
            "migration 20260729-01: 清理 platform.extra 里的 W2 peak_hours 历史副本"
        );
    }
}
...
fn rename_extra_time_models_to_windows(conn: &Connection) {
    let rows: Vec<(i64, String)> =
        match conn.prepare("SELECT id, extra FROM platform WHERE extra LIKE '%\"time_models\"%'") {
            Ok(mut stmt) => stmt
                .query_map([], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?)))
                .ok()
                .map(|iter| iter.filter_map(Result::ok).collect())
    ...
    if renamed > 0 {
        tracing::info!(
            renamed,
            "migration 20260829-01: platform.extra.time_models → time_windows 改名完成"
        );
    }
}
...
fn rename_extra_peak_hours_to_peak(conn: &Connection) {
    ...
    if renamed > 0 {
        tracing::info!(
            renamed,
            "migration 20260829-02: platform.extra.peak_hours → peak 改名完成"
        );
    }
}
...
fn reextract_legacy_last_error(conn: &Connection) {
    ...
    let Ok(mut stmt) = conn.prepare("SELECT id, last_error FROM platform WHERE last_error != ''")
    else {
        return;
    };
...
fn migrate_mitm_legacy_tables_to_setting(conn: &Connection) {
    ...
    if let Ok(mut stmt) =
        conn.prepare("SELECT base_url FROM platform WHERE deleted_at = 0 AND base_url != ''")
    {
        ...
        for base_url in hosts {
            if let Some(host) = crate::endpoint_host(&base_url) {
                ...
                let dup = entries
                    .iter()
                    .any(|e| e.get("host_pattern").and_then(|v| v.as_str()) == Some(host.as_str()));
                if !dup {
                    entries.push(serde_json::json!({
                        "host_pattern": host,
                        "rule_type": "domain",
...
mod tests {
    ...
    fn migrations_late_group_path_migration_executed() {
        ...
        assert!(
            result.is_ok(),
            "run_migrations_platform_late failed: {:?}",
            result
        );
    ...
    fn migrations_middleware_rule_unified_model_20260824() {
        ...
        assert!(
            !cols.contains(&"rule_type".to_string()),
            "old columns dropped"
        );
        ...
        assert!(
            cond.contains("\"kind\":\"all\"")
                && cond.contains("\"target\":\"request_body\"")
                && cond.contains("\"match_type\":\"contains\"")
        );
        ...
        assert!(
            acts.contains("\"replacement\":\"[x]\"") && acts.contains("\"fields\":[\"messages\"]")
        );
        assert!(appl.contains("\"platforms\":[7]"));

        // ③ 空 pattern 检测器行 → 显式 secret/email any 组；enabled=0 保留
        let (cond, enabled): (String, i64) = conn
            .query_row(
        ...
        let failed: i64 = conn
            .query_row(
                "SELECT failed FROM middleware_rule WHERE name='user-unknown'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let ghost: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM middleware_rule WHERE name='user-ghost-builtin'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        assert_eq!(failed, 0);
        assert!(
            cond.contains("\"kind\":\"leaf\""),
            "builtin content overwritten to new spec"
        );
        // 新内置规格补齐（升级新增的 DB/Redis 脱敏等）
        let n_builtin: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM middleware_rule WHERE is_builtin=1",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(
            n_builtin as usize,
            crate::schema::builtin_rule_specs().len()
        );
    ...
    fn migrations_late_modern_schema_idempotent() {
        ...
        assert!(
            result.is_ok(),
            "modern schema migration should succeed: {:?}",
            result
        );
    }
    ...
    fn migrations_late_group_extra_column_044() {
        ...
        assert!(
            r1.is_ok(),
            "first migration 20260727-13 should succeed: {:?}",
            r1
        );
        let has_extra = conn
            .prepare("PRAGMA table_info(\"group\")")
        ...
        assert!(
            has_extra,
            "extra column must exist after migration 20260727-13"
        );
        // 行数据保留 + extra 默认 ''（空串 = "{}" 轻量表示）
        let extra: String = conn
            .query_row("SELECT extra FROM \"group\" WHERE name='g044'", [], |r| {
                r.get(0)
            })
            .unwrap();
        ...
        assert!(
            r2.is_ok(),
            "re-running migration 20260727-13 must be idempotent: {:?}",
            r2
        );
    }
    ...
    fn migrations_late_breaker_backfill_exercises_026() {
        ...
        assert!(
            result.is_ok(),
            "breaker backfill migration should succeed: {:?}",
            result
        );
        ...
        assert!(
            !has_breaker,
            "breaker_failure_threshold should be dropped after migration 20260727-06"
        );
        // The non-zero platform's extra should now contain breaker data.
        let extra: String = conn
            .query_row(
                "SELECT extra FROM platform WHERE name = 'test-plat'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert!(
            extra.contains("breaker") || extra.contains("failure_threshold"),
            "extra should contain breaker data after backfill, got: {}",
            extra
        );
    }
    ...
    fn migrations_late_glm_coding_plan_backfill_025() {
        ...
        assert!(
            result.is_ok(),
            "GLM coding_plan migration should succeed: {:?}",
            result
        );
        // After migration, anthropic endpoint should have coding_plan=true.
        let ep_json: String = conn
            .query_row(
                "SELECT endpoints FROM platform WHERE name = 'GLM Test'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        let eps: Vec<serde_json::Value> = serde_json::from_str(&ep_json).unwrap();
        let anthropic_ep = eps
            .iter()
            .find(|ep| ep.get("protocol").and_then(|v| v.as_str()) == Some("anthropic"))
            .unwrap();
        assert_eq!(
    ...
    fn migrations_proxy_log_done_column_backfill_20260824() {
        ...
        let done: i64 = conn
            .query_row("SELECT done FROM proxy_log WHERE id='term_ok'", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(done, 1, "历史 200 终态行应回填 done=1");
        let done: i64 = conn
            .query_row("SELECT done FROM proxy_log WHERE id='term_err'", [], |r| {
                r.get(0)
            })
            .unwrap();
        ...
        let (done, body): (i64, String) = conn
            .query_row(
                "SELECT done, response_body FROM proxy_log WHERE id='stuck'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        ...
        let done: i64 = conn
            .query_row("SELECT done FROM proxy_log WHERE id='mid'", [], |r| {
                r.get(0)
            })
            .unwrap();
    ...
    fn migrations_proxy_log_field_trace_column_20260827() {
        ...
        let ft: String = conn
            .query_row(
                "SELECT field_trace FROM proxy_log WHERE id='legacy'",
                [],
                |r| r.get(0),
            )
        ...
        conn.execute(
            "UPDATE proxy_log SET field_trace = 'drop:user' WHERE id='legacy'",
            [],
        )
        .unwrap();
        run_migrations_proxy_log_late(&conn, &std::collections::HashMap::new(), &[], &[])
    ...
    fn migrations_late_reextract_last_error_039() {
        ...
        let get_last_error = |name: &str| -> String {
            conn.query_row(
                "SELECT last_error FROM platform WHERE name = ?1",
                [name],
                |r| r.get(0),
            )
            .unwrap()
    ...
    fn migrations_late_mitm_seed_to_setting_043() {
        ...
        assert!(
            !has_mitm_ca,
            "mitm_ca table must NOT exist (migrated to setting)"
        );
        let has_mitm_whitelist = table_exists(&conn, "mitm_whitelist");
        assert!(
            !has_mitm_whitelist,
            "mitm_whitelist table must NOT exist (migrated to setting)"
        );
        ...
        assert!(
            arr.len() >= 37,
            "default whitelist should contain 37 Clash ruleset entries, got {}",
            arr.len()
        );
        ...
        assert!(
            has_platform_host,
            "platform base_url host 'api.anthropic.com' (domain) should be seeded"
        );
        ...
        assert_eq!(
            entries2.as_array().unwrap().len(),
            arr.len(),
            "re-running migration should not duplicate whitelist entries"
        );
        ...
        for (rule_type, expected) in [
            ...
            assert!(
                has,
                "default whitelist should contain rule_type={rule_type} host_pattern={expected}"
            );
        }
    }
    ...
    fn migrations_late_043_legacy_tables_to_setting() {
        ...
        // ① 两表已 DROP
        assert!(
            !table_exists(&conn, "mitm_ca"),
            "mitm_ca must be DROPped after migration"
        );
        assert!(
            !table_exists(&conn, "mitm_whitelist"),
            "mitm_whitelist must be DROPped after migration"
        );
        ...
        assert_eq!(
            ca.get("private_key_pem").and_then(|v| v.as_str()),
            Some("TEST_PRIV_KEY")
        );
        assert_eq!(
            ca.get("cert_pem").and_then(|v| v.as_str()),
            Some("TEST_CERT_PEM")
        );
        assert_eq!(
            ca.get("fingerprint").and_then(|v| v.as_str()),
            Some("AB:CD")
        );
        ...
        assert_eq!(
            arr.len(),
            3,
            "legacy whitelist (non-empty) should migrate as-is, seed skipped"
        );
        // 验 created_at 升序：第一条 anthropic.com（created_at=100），第二条 api.openai.com（101）
        assert_eq!(
            arr[0].get("host_pattern").and_then(|v| v.as_str()),
            Some("anthropic.com")
        );
        assert_eq!(
            arr[1].get("host_pattern").and_then(|v| v.as_str()),
            Some("api.openai.com")
        );
        // 验 rule_type / enabled / source 字段迁移正确
        assert_eq!(
            arr[0].get("rule_type").and_then(|v| v.as_str()),
            Some("suffix")
        );
        ...
        assert_eq!(
            ca_count2, 1,
            "re-running migration must not duplicate mitm:ca"
        );
        ...
        assert_eq!(
            wl2.as_array().unwrap().len(),
            3,
            "re-running migration must not change whitelist"
        );
    }
    ...
    fn migrations_platform_extra_time_models_renamed_20260829() {
        ...
        let extra_of = |id: i64| -> serde_json::Value {
            let raw: String = conn
                .query_row(
                    "SELECT extra FROM platform WHERE id = ?1",
                    params![id],
                    |r| r.get(0),
                )
                .unwrap();
        ...
        let e1 = extra_of(1);
        assert_eq!(
            e1["time_windows"][0]["models"]["default"], "m1",
            "old-key row: value moved to time_windows"
        );
        assert!(
            e1.get("time_models").is_none(),
            "old-key row: time_models must be removed"
        );
        assert_eq!(
            e1["breaker"]["threshold"], 3,
            "unrelated key breaker preserved"
        );
        // ② 新旧并存 → 新键优先，旧键丢弃
        let e2 = extra_of(2);
        assert_eq!(
            e2["time_windows"][0]["models"]["default"], "new",
            "both-keys row: new key wins"
        );
        assert!(
            e2.get("time_models").is_none(),
            "both-keys row: old key dropped"
        );
        // ③ 本就只有新键 → 原样（空转路径）
        let e3 = extra_of(3);
        assert_eq!(
            e3["time_windows"][0]["models"]["default"], "kept",
            "new-key-only row untouched"
        );
        assert_eq!(e3["mock"]["v"], 1, "unrelated key mock preserved");
        // ④ 无任一键 → 完全不动
        let e4 = extra_of(4);
        assert!(
            e4.get("time_windows").is_none() && e4.get("time_models").is_none(),
            "unrelated row gains no key"
        );
        assert_eq!(
            e4["disable_during_peak"], true,
            "unrelated row values intact"
        );
        // ④b 值里含裸词 time_models（非带引号键）→ LIKE 不误命中，行不动
        let e5 = extra_of(5);
        assert!(
            e5.get("time_windows").is_none() && e5.get("time_models").is_none(),
            "value-substring row gains no key"
        );
        assert!(
            e5["note"].as_str().unwrap().contains("time_models"),
            "value-substring row note preserved"
        );
    ...
    fn migrations_platform_extra_peak_hours_renamed_20260829() {
        ...
        let e1 = extra_of(1);
        assert_eq!(
            e1["peak"][0]["multiplier"], 3.0,
            "old-key row: value moved to peak"
        );
        assert!(
            e1.get("peak_hours").is_none(),
            "old-key row: peak_hours must be removed"
        );
        assert_eq!(
            e1["breaker"]["threshold"], 3,
            "unrelated key breaker preserved"
        ...
        let e2 = extra_of(2);
        assert_eq!(
            e2["peak"][0]["multiplier"], 1.5,
            "both-keys row: new key wins"
        );
        assert!(
            e2.get("peak_hours").is_none(),
            "both-keys row: old key dropped"
        );
        // ③ 本就只有新键 → 原样（空转路径）
        let e3 = extra_of(3);
        assert_eq!(
            e3["peak"][0]["multiplier"], 1.25,
            "new-key-only row untouched"
        );
        assert_eq!(e3["mock"]["v"], 1, "unrelated key mock preserved");
        // ④ 无任一键 → 完全不动
        let e4 = extra_of(4);
        assert!(
            e4.get("peak").is_none() && e4.get("peak_hours").is_none(),
            "unrelated row gains no key"
        );
        assert_eq!(
            e4["disable_during_peak"], true,
        ...
        let e5 = extra_of(5);
        assert!(
            e5.get("peak").is_none() && e5.get("peak_hours").is_none(),
            "value-substring row gains no key"
        );
        assert!(
            e5["note"].as_str().unwrap().contains("peak_hours"),
            "value-substring row note preserved"
        );
