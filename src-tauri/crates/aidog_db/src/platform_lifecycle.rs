...
#[allow(dead_code)]
#[track_caller]
pub fn update_platform_quota<'a>(
    db: &'a Db,
    id: u64,
    balance: f64,
    coding_plan_json: &'a str,
    ...
    async move {
        ...
        match group_id {
            ...
            None => {
                ...
                Ok(rows
                    .into_iter()
                    .map(|(id, name, reason)| PurgeCandidate {
                        id: id as u64,
                        name,
                        reason,
                        action: "delete",
...
#[allow(dead_code)]
#[track_caller]
pub fn purge_old_soft_deleted_platforms(
    db: &Db,
    older_than_secs: i64,
) -> impl std::future::Future<Output = Result<u64, String>> + '_ {
    let __db_caller = std::panic::Location::caller();
    async move {
...
pub fn set_tray_platform<'a>(
    ...
    let __db_caller = std::panic::Location::caller();
    async move {
        let display = if tray_display == "coding" {
            "coding"
        } else {
            "balance"
        }
...
pub fn clear_tray(db: &Db) -> impl std::future::Future<Output = Result<(), String>> + '_ {
    ...
    async move {
        db.call_platform_traced(None, __db_caller, move |conn| {
            conn.execute(
                "UPDATE platform SET show_in_tray = 0, updated_at = ?1 WHERE show_in_tray = 1",
                params![now()],
            )?;
            Ok(())
        })
