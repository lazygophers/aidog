...
async fn r9_soft_delete_platform() {
    ...
    let deleted_at: i64 = db
        .call_traced(None, std::panic::Location::caller(), move |conn| {
            Ok(conn.query_row(
                "SELECT deleted_at FROM platform WHERE id = ?1",
                params![pid],
                |r| r.get(0),
            )?)
        })
...
async fn delete_platform_preserves_groups_with_other_members() {
    let db = test_db().await;
    let p_del = create_platform(&db, sample_platform("del-src"))
        .await
        .unwrap();
    let p_keep = create_platform(&db, sample_platform("keep")).await.unwrap();

    // ① 手动组：同时含待删平台与另一存活平台。
    let m = create_group(&db, sample_group("m-shared", vec![]))
        .await
        .unwrap();
    set_group_platforms(
        &db,
        ...
        &[
            GroupPlatformInput {
                platform_id: p_del.id,
                priority: Some(0),
                weight: Some(1),
                level_priority: None,
            },
            GroupPlatformInput {
                platform_id: p_keep.id,
                priority: Some(1),
                weight: Some(1),
                level_priority: None,
            },
    ...
    let auto_g = create_group(
        &db,
        CreateGroup {
            name: "del-src-auto".into(),
            group_key: Some("del-src-auto".into()),
            routing_mode: RoutingMode::Failover,
            auto_from_platform: p_del.id.to_string(),
    ...
    let auto_orphan = create_group(
        &db,
        CreateGroup {
            name: "del-src-orphan".into(),
            group_key: Some("del-src-orphan".into()),
            routing_mode: RoutingMode::Failover,
            auto_from_platform: p_del.id.to_string(),
            request_timeout_secs: 0,
            connect_timeout_secs: 0,
    ...
    assert!(
        get_group(&db, m.id).await.unwrap().is_some(),
        "手动组不应被删"
    );
    ...
    assert!(
        get_group(&db, auto_g.id).await.unwrap().is_some(),
        "有其他成员的 auto 组不应被删"
    );
    ...
    assert!(
        get_group(&db, auto_orphan.id).await.unwrap().is_some(),
        "孤儿 auto 组应保留"
    );
...
async fn purge_auto_disabled_global_deletes_all() {
    ...
    let p_dead1 = create_platform(&db, sample_platform("dead1"))
        .await
        .unwrap();
    let p_dead2 = create_platform(&db, sample_platform("dead2"))
        .await
        .unwrap();
    ...
    set_platform_last_error(&db, p_dead1.id, Some("HTTP 401: unauthorized".into()))
        .await
        .unwrap();
    set_platform_last_error(&db, p_dead2.id, Some("HTTP 403: forbidden".into()))
        .await
        .unwrap();
    ...
    assert!(
        get_platform(&db, p_alive.id).await.unwrap().is_some(),
        "enabled 平台不应被删"
    );
}
...
async fn purge_auto_disabled_group_exclusive_deletes_shared_unassigns() {
    ...
    let p_a = create_platform(&db, sample_platform("a-exclusive"))
        .await
        .unwrap();
    ...
    set_platform_last_error(&db, p_a.id, Some("HTTP 401: bad key".into()))
        .await
        .unwrap();
    set_platform_last_error(&db, p_b.id, Some("HTTP 403: forbidden".into()))
        .await
        .unwrap();
    ...
    let r = purge_auto_disabled_platforms(&db, Some(g1.id))
        .await
        .unwrap();
    ...
    // A 已软删；B 行仍在。
    assert!(
        get_platform(&db, p_a.id).await.unwrap().is_none(),
        "A 应被软删"
    );
    assert!(
        get_platform(&db, p_b.id).await.unwrap().is_some(),
        "B 行应保留"
    );
...
async fn purge_auto_disabled_group_skips_enabled() {
    ...
    let p_alive = create_platform(&db, sample_platform("alive-g"))
        .await
        .unwrap();
    ...
    set_group_platforms(
        ...
        &[
            GroupPlatformInput {
                platform_id: p_alive.id,
                priority: Some(0),
                weight: Some(1),
                level_priority: None,
            },
            GroupPlatformInput {
                platform_id: p_dead.id,
                priority: Some(1),
                weight: Some(1),
                level_priority: None,
            },
    ...
    let r = purge_auto_disabled_platforms(&db, Some(g.id))
        .await
        .unwrap();
    assert_eq!(
        r.deleted_ids,
        vec![p_dead.id as u64],
        "仅 auto_disabled 应被删"
    );
...
async fn tray_config_migrate_from_legacy_platform() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("legacy"))
        .await
        .unwrap();
...
async fn purge_global_also_deletes_expired_platforms() {
    ...
    let p_future = create_platform(&db, sample_platform("future"))
        .await
        .unwrap();
    let p_disabled = create_platform(&db, sample_platform("disabled"))
        .await
        .unwrap();
    let now = now();
    ...
    assert!(
        r.deleted_ids.contains(&(p_expired1.id as u64)),
        "过期1 应删"
    );
    assert!(
        r.deleted_ids.contains(&(p_expired2.id as u64)),
        "过期2 应删"
    );
    assert!(
        r.deleted_ids.contains(&(p_disabled.id as u64)),
        "auto_disabled 应删"
    );
    assert!(
        get_platform(&db, p_future.id).await.unwrap().is_some(),
        "未过期平台应保留"
    );
}
...
async fn purge_group_also_deletes_expired_platforms() {
    ...
    let p_exp_excl = create_platform(&db, sample_platform("exp-excl"))
        .await
        .unwrap();
    // p_exp_shared：属 g1 + g2，过期 → 仅删 g1 关联，平台行保留。
    let p_exp_shared = create_platform(&db, sample_platform("exp-shared"))
        .await
        .unwrap();
    // p_alive：属 g1，未过期 → 保留。
    let p_alive = create_platform(&db, sample_platform("alive"))
    ...
    set_group_platforms(
        ...
        &[
            GroupPlatformInput {
                platform_id: p_exp_excl.id,
                priority: Some(0),
                weight: Some(1),
                level_priority: None,
            },
            GroupPlatformInput {
                platform_id: p_exp_shared.id,
                priority: Some(1),
                weight: Some(1),
                level_priority: None,
            },
    ...
    assert_eq!(
        r.deleted_ids,
        vec![p_exp_excl.id as u64],
        "独占过期平台应永久删"
    );
    assert_eq!(
        r.unassigned_ids,
        vec![p_exp_shared.id as u64],
        "共享过期平台应仅移关联"
    );
    assert!(
        get_platform(&db, p_exp_excl.id).await.unwrap().is_none(),
        "独占过期平台行应软删"
    );
    assert!(
        get_platform(&db, p_exp_shared.id).await.unwrap().is_some(),
        "共享过期平台行应保留"
    );
    // p_alive 仍在 g1
    let g1_plats = get_group_platforms(&db, g1.id).await.unwrap();
...
async fn update_platform_quota_persists() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("quota-test"))
        .await
        .unwrap();
    update_platform_quota(&db, p.id, 12.34, r#"{"plan":"pro"}"#)
        .await
        .unwrap();
    let got = get_platform(&db, p.id)
        .await
        .unwrap()
...
async fn purge_old_soft_deleted_removes_expired_rows() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("purge-old"))
        .await
        .unwrap();
    // 软删
    delete_platform(&db, p.id).await.unwrap();
    ...
    assert!(
        n >= 1,
        "should delete at least 1 old soft-deleted platform, got {n}"
    );
    // 行已被物理删除，无法按 id 查到（get_platform 只过滤 deleted_at=0）
    let pid = p.id as i64;
    let exists: i64 = db
        .call_traced(None, std::panic::Location::caller(), move |c| {
            Ok(c.query_row(
                "SELECT COUNT(*) FROM platform WHERE id = ?1",
                params![pid],
                |r| r.get(0),
            )?)
        })
...
async fn purge_old_soft_deleted_keeps_recent_rows() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("purge-recent"))
        .await
        .unwrap();
    delete_platform(&db, p.id).await.unwrap();
    // older_than_secs=9999999 → cutoff 在未来，deleted_at 不满足 < cutoff
    let n = purge_old_soft_deleted_platforms(&db, 9_999_999)
        .await
        .unwrap();
...
async fn tray_platform_set_get_clear() {
    let db = test_db().await;
    let p1 = create_platform(&db, sample_platform("tray-p1"))
        .await
        .unwrap();
    let p2 = create_platform(&db, sample_platform("tray-p2"))
        .await
        .unwrap();
    ...
    let got = get_tray_platform(&db)
        .await
        .unwrap()
        .expect("should have tray platform");
    ...
    let got2 = get_tray_platform(&db)
        .await
        .unwrap()
        .expect("p2 should be tray");
    assert_eq!(got2.id, p2.id);
    assert_eq!(got2.tray_display, "coding");

    // 验证 p1 已无 show_in_tray
    let p1_row = get_platform(&db, p1.id)
        .await
        .unwrap()
        .expect("p1 still exists");
...
async fn purge_keeps_recoverable_auto_disabled() {
    ...
    set_platform_last_error(&db, p_401.id, Some("HTTP 401: unauthorized".into()))
        .await
        .unwrap();
    set_platform_last_error(&db, p_402.id, Some("HTTP 402: 余额不足".into()))
        .await
        .unwrap();
    set_platform_last_error(
        &db,
        p_429q.id,
        Some("HTTP 429: 已达到 Token Plan 用量上限".into()),
    )
    .await
    .unwrap();
    ...
    assert!(
        get_platform(&db, p_402.id).await.unwrap().is_some(),
        "402 可恢复平台应保留"
    );
    assert!(
        get_platform(&db, p_429q.id).await.unwrap().is_some(),
        "429-配额可恢复平台应保留"
    );
}
...
async fn preview_candidates_match_purge_result_global_and_group() {
    // ── 全局模式 ──
    {
        ...
        assert!(
            candidates.iter().all(|c| c.action == "delete"),
            "全局模式 action 恒为 delete"
        );
        ...
        assert_eq!(
            ...
            "全局：preview id 集合应等于 purge 实际处理集合"
        );
        assert!(r.unassigned_ids.is_empty());
    }
    ...
    // ── 分组模式（独占删 + 共享移关联）──
    {
        ...
        let candidate_ids: std::collections::HashSet<u64> =
            candidates.iter().map(|c| c.id).collect();
        let candidate_action_of =
            |id: u64| candidates.iter().find(|c| c.id == id).map(|c| c.action);
        assert_eq!(
            candidate_action_of(p_a.id as u64),
            Some("delete"),
            "独占的 A 应标 delete"
        );
        assert_eq!(
            candidate_action_of(p_b.id as u64),
            Some("unassign"),
            "共享的 B 应标 unassign"
        );
        ...
        assert_eq!(
            candidate_ids, actual_ids,
            "分组：preview id 集合应等于 purge 实际处理集合"
        );

        // 每项 action 与它实际落到哪个集合一致。
        for id in &r.deleted_ids {
            assert_eq!(
                candidate_action_of(*id),
                Some("delete"),
                "落入 deleted_ids 的项 preview action 应为 delete"
            );
        }
        for id in &r.unassigned_ids {
            assert_eq!(
                candidate_action_of(*id),
                Some("unassign"),
                "落入 unassigned_ids 的项 preview action 应为 unassign"
            );
        }
    }
}
