...
async fn r4_group_model_mappings_inline_roundtrip() {
    ...
    let g = create_group(&db, sample_group("mm", mappings))
        .await
        .unwrap();
...
async fn create_and_list_groups() {
    let db = test_db().await;
    let g1 = create_group(&db, sample_group("group-a", vec![]))
        .await
        .unwrap();
    let g2 = create_group(&db, sample_group("group-b", vec![]))
        .await
        .unwrap();
    let groups = list_groups(&db).await.unwrap();
...
async fn update_group_changes_name() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("original", vec![]))
        .await
        .unwrap();
...
async fn update_group_changes_routing_mode() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("routing-test", vec![]))
        .await
        .unwrap();
    let updated = update_group(
        &db,
...
async fn update_group_timeouts() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("timeout-test", vec![]))
        .await
        .unwrap();
    let updated = update_group(
        &db,
...
async fn delete_group_non_auto() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("to-delete", vec![]))
        .await
        .unwrap();
...
async fn reorder_platforms_works() {
    let db = test_db().await;
    let p1 = create_platform(&db, sample_platform("plat-a"))
        .await
        .unwrap();
    let p2 = create_platform(&db, sample_platform("plat-b"))
        .await
        .unwrap();
...
async fn reorder_group_platforms_works() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("gp-reorder", vec![]))
        .await
        .unwrap();
    let p1 = create_platform(&db, sample_platform("p-reorder-1"))
        .await
        .unwrap();
    let p2 = create_platform(&db, sample_platform("p-reorder-2"))
        .await
        .unwrap();
    set_group_platforms(
        &db,
    ...
    .await
    .unwrap();
    reorder_group_platforms(&db, g.id, &[p2.id, p1.id])
        .await
        .unwrap();
...
async fn set_group_platform_level_priority_updates() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("lp-group", vec![]))
        .await
        .unwrap();
    let p = create_platform(&db, sample_platform("lp-platform"))
        .await
        .unwrap();
    set_group_platforms(
        &db,
    ...
    .await
    .unwrap();
    set_group_platform_level_priority(&db, g.id, p.id, 7)
        .await
        .unwrap();
    // No error = success
}
...
async fn set_group_platform_level_priority_clamped() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("lp-clamp", vec![]))
        .await
        .unwrap();
    let p = create_platform(&db, sample_platform("lp-clamp-p"))
        .await
        .unwrap();
    set_group_platforms(
        &db,
    ...
    set_group_platform_level_priority(&db, g.id, p.id, 100)
        .await
        .unwrap();
    set_group_platform_level_priority(&db, g.id, p.id, -5)
        .await
        .unwrap();
}
...
async fn move_group_platform_between_groups() {
    let db = test_db().await;
    let g1 = create_group(&db, sample_group("from-group", vec![]))
        .await
        .unwrap();
    let g2 = create_group(&db, sample_group("to-group", vec![]))
        .await
        .unwrap();
    let p = create_platform(&db, sample_platform("movable-plat"))
        .await
        .unwrap();
    set_group_platforms(
        &db,
    ...
    assert!(
        g1_plats.is_empty(),
        "platform should be removed from source group"
    );
    assert_eq!(
        g2_plats.len(),
        1,
        "platform should be added to target group"
    );
}
...
async fn create_group_auto_generates_group_key() {
    ...
    let g = create_group(&db, input).await.unwrap();
    assert!(
        g.group_key.starts_with("gk_"),
        "auto-generated key should start with gk_: {}",
        g.group_key
    );
