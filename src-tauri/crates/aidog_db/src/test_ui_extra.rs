...
async fn ui_extra_empty_to_json_with_key() {
    ...
    let p = create_platform(&db, sample_platform("ui-empty"))
        .await
        .unwrap();
    update_extra_key(
        &db,
...
async fn ui_extra_preserves_existing_business_keys() {
    ...
    input.extra =
        r#"{"breaker":{"failure_threshold":3,"open_secs":30,"half_open_max":1,"recovery_secs":5}}"#
            .to_string();
...
async fn ui_extra_overwrite_same_key() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("ui-ow"))
        .await
        .unwrap();
    update_extra_key(
        &db,
...
async fn ui_extra_group_round_trip() {
    let db = test_db().await;
    let g = create_group(&db, sample_group("ui-group", vec![]))
        .await
        .unwrap();
