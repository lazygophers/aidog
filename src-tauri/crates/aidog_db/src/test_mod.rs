...
fn parse_models_corrupt_json_returns_default() {
    ...
    assert!(
        m.default.is_none(),
        "corrupt JSON should fall back to empty default"
    );
}
...
async fn call_traced_reopens_after_panic_kills_thread() {
    ...
    let r_kill: tokio_rusqlite::Result<()> = db
        .write_conn()
        .call(|_c| panic!("test: kill write thread"))
        .await;
    drop(_hook);
...
async fn call_traced_skips_reopen_for_memory_db() {
    ...
    let _hook = QuietPanicHook::install();
    let _: tokio_rusqlite::Result<()> = db
        .write_conn()
        .call(|_c| panic!("test: kill memory write thread"))
        .await;
    drop(_hook);
