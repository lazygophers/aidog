...
async fn upsert_roundtrip_preserves_all_columns() {
    ...
    let got = get_model_entry(&db, "glm", "glm-4.6")
        .await
        .unwrap()
        .expect("row");
    ...
    assert_eq!(
        got.capabilities,
        vec!["text".to_string(), "tool_use".to_string()]
    );
...
async fn upsert_is_idempotent_on_composite_key() {
    ...
    upsert_model_entries(&db, vec![e.clone(), e.clone()])
        .await
        .unwrap();
    ...
    upsert_model_entries(&db, vec![entry("aihubmix", "glm-4.6", "glm-4.6", false)])
        .await
        .unwrap();
...
async fn platform_preset_upsert_overwrites_whole_json() {
    ...
    upsert_platform_presets(
        ...
        vec![PlatformPreset {
            code: "glm".into(),
            preset_data: r##"{"color":"#111111"}"##.into(),
            updated_at: 0,
        }],
    )
    .await
    ...
    upsert_platform_presets(
        ...
        vec![PlatformPreset {
            code: "glm".into(),
            preset_data: r##"{"color":"#222222"}"##.into(),
            updated_at: 0,
        }],
    )
    .await
    ...
    let rows = list_platform_presets(&db).await.unwrap();
    assert_eq!(
        rows.iter().find(|p| p.code == "glm").unwrap().preset_data,
        r##"{"color":"#222222"}"##
    );
}
...
async fn bundled_fallback_serves_empty_db() {
    ...
    let got = get_model_entry(&db, &sample.platform_code, &sample.model_id)
        .await
        .unwrap();
...
async fn display_name_roundtrips_and_falls_back_to_model_id() {
    ...
    upsert_model_entries(&db, vec![named, blank, spaces])
        .await
        .unwrap();
    ...
    assert_eq!(
        get_model_entry(&db, "glm", "glm-4.5")
            .await
            .unwrap()
            .unwrap()
            .display_name,
    ...
    assert_eq!(
        get_model_entry(&db, "glm", "glm-4.4")
            .await
            .unwrap()
            .unwrap()
            .display_name,
    ...
    assert!(
        list_model_entries(&db, Some("glm"))
            .await
            .unwrap()
            .iter()
            .all(|e| !e.display_name.is_empty())
    );
    ...
    assert_eq!(
        get_model_entry(&db, "glm", "glm-4.6")
            .await
            .unwrap()
            .unwrap()
            .display_name,
...
fn group_display_name_comes_from_official_entry() {
    ...
    assert_eq!(
        groups[0].display_name, "GLM-4.6",
        "聚合行取 official 那条的展示名"
    );
    ...
    let groups = group_by_canonical(
        vec![entry("glm", "glm-4.5", "glm-4.5", true)],
        &Default::default(),
    );
...
fn from_json_reads_display_name_without_write_side_fallback() {
    let e = model_entry_from_json("glm", r#"{"model_id":"glm-4.6","display_name":"GLM-4.6"}"#)
        .expect("parse");
...
fn from_json_defaults_canonical_to_model_id() {
    ...
    assert!(
        model_entry_from_json("glm", r#"{"canonical_model":"x"}"#).is_none(),
        "缺 model_id 跳过"
    );
...
fn bundled_registry_entries_are_wellformed() {
    ...
    assert!(
        entries.len() > 500,
        "registry 现有 900+ 模型条目，实测 {}",
        entries.len()
    );
    assert!(
        entries
            .iter()
            .all(|e| !e.model_id.is_empty() && !e.canonical_model.is_empty())
    );
    ...
    assert!(
        presets.windows(2).all(|w| w[0].code < w[1].code),
        "code 升序且无重复"
    );
}
...
async fn presets_doc_json_prefers_db_over_bundled() {
    ...
    let doc: serde_json::Value =
        serde_json::from_str(&presets_doc_json(&db).await.unwrap()).unwrap();
    let protocols = doc["protocols"].as_object().expect("protocols");
    assert_eq!(
        protocols["glm"]["name"]["en-US"], "Zhipu Renamed",
        "同 code 以 DB 行为准"
    );
    // 票 13-C：DB 里没有的协议由 bundled 补齐，不能整篇接管
    assert_eq!(protocols.len(), bundled_platform_presets().len());
    assert!(
        protocols["anthropic"]["homepage"].is_string(),
        "DB 缺的协议须由 bundled 补齐"
    );
    assert!(
        doc["last_updated"].as_i64().unwrap_or(0) > 0,
        "last_updated 取行 updated_at 最大值（秒）"
    );
...
async fn preset_entry_reads_single_row_with_bundled_fallback() {
    let db = test_db().await;
    assert!(
        preset_entry(&db, "anthropic").await.unwrap().is_some(),
        "DB 空 → 回落 bundled"
    );
    assert!(
        preset_entry(&db, "__never_exists__")
            .await
            .unwrap()
            .is_none()
    );
    ...
    .await
    .unwrap();
    let v = preset_entry(&db, "glm_coding")
        .await
        .unwrap()
        .expect("entry");
    assert_ne!(v["logo_url"], "stale", "旧行不得盖掉更新的 bundled");
    assert!(
        v["quota_scripts"].is_array(),
        "bundled 的 quota_scripts 须仍可见"
    );
}
...
async fn bundled_fallback_is_per_key_not_whole_table() {
    ...
    upsert_model_entries(
        &db,
        vec![entry("zzz-only", "zzz-model", "zzz-model", false)],
    )
    .await
    .unwrap();
    ...
    let got = get_model_entry(&db, &sample.platform_code, &sample.model_id)
        ...
        .unwrap();
    assert_eq!(
        got.map(|e| e.model_id),
        Some(sample.model_id.clone()),
        "该键 DB 缺失 → 回落 bundled"
    );
    assert!(
        get_model_entry(&db, "zzz-only", "no-such-model")
            .await
            .unwrap()
            .is_none()
    );
...
fn primary_platform_skips_pricing_only_sources() {
    let pricing_only: std::collections::HashSet<String> =
        ["mistral".to_string()].into_iter().collect();
    let mut official = entry("mistral", "codestral-2", "codestral-2", true);
    official.display_name = "Codestral".into();
    let groups = group_by_canonical(
        vec![
            official.clone(),
            entry("siliconflow", "codestral-2", "codestral-2", false),
        ],
        &pricing_only,
    );
    assert_eq!(
        groups[0].primary_platform, "siliconflow",
        "official 但不可选 → 让给可选平台"
    );
...
async fn select_returns_raw_display_name_for_export() {
    let db = test_db().await;
    upsert_model_entries(&db, vec![entry("glm", "glm-4.5", "glm-4.5", true)])
        .await
        .unwrap();
