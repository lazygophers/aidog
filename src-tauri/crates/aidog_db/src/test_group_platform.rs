...
async fn list_group_details_cache_invalidation() {
    ...
    assert_eq!(
        d2[0].platforms[0].platform.name, "P1-renamed",
        "update_platform 后缓存仍旧名 → 失效漏"
    );
    ...
    let before = list_group_details(&db).await.unwrap()[0].platforms[0]
        .platform
        .est_balance_remaining;
    assert!(
        (before - 100.0).abs() < 1e-6,
        "quota 写后缓存须见 100，实得 {before}"
    );
    crate::apply_balance_delta(&db, p1.id, 30.0).await.unwrap();
    let after = list_group_details(&db).await.unwrap()[0].platforms[0]
        .platform
        .est_balance_remaining;
    assert!(
        (after - 70.0).abs() < 1e-6,
        "apply_balance_delta 后缓存仍旧值 → 失效漏，实得 {after}"
    );

    // ④ set_group_platform_level_priority → level_priority 须刷新。
    set_group_platform_level_priority(&db, g.id, p1.id, 9)
        .await
        .unwrap();
    let d4 = list_group_details(&db).await.unwrap();
    assert_eq!(
        d4[0].platforms[0].level_priority, 9,
        "level_priority 写后缓存仍旧 → 失效漏"
    );
...
async fn get_group_platforms_cache_invalidation() {
    ...
    let g = create_group(&db, sample_group("gpc", vec![]))
        .await
        .unwrap();
    ...
    assert_eq!(
        cached_after_delete.len(),
        1,
        "delete_platform 后缓存仍含已删平台 → 会转发到已删平台"
    );
...
async fn r4_group_detail_mappings_from_group_field() {
    ...
    let g = create_group(&db, sample_group("d", mappings))
        .await
        .unwrap();
    ...
    assert_eq!(
        detail.model_mappings.len(),
        detail.group.model_mappings.len()
    );
    assert_eq!(
        detail.model_mappings[0].source_model,
        detail.group.model_mappings[0].source_model
    );
    assert_eq!(
        detail.model_mappings[0].target_platform_id,
        detail.group.model_mappings[0].target_platform_id
    );
    assert_eq!(
        detail.model_mappings[0].target_model,
        detail.group.model_mappings[0].target_model
    );
}
...
async fn sync_platform_manual_groups_adds_removes_preserves_auto() {
    let db = test_db().await;
    let p = create_platform(&db, sample_platform("sync-p"))
        .await
        .unwrap();
    ...
    sync_platform_manual_groups(&db, p.id, &[m1.id])
        .await
        .unwrap();
    assert_eq!(get_group_platforms(&db, m1.id).await.unwrap().len(), 1);
    assert_eq!(get_group_platforms(&db, m2.id).await.unwrap().len(), 0);
    assert_eq!(
        get_group_platforms(&db, auto_g.id).await.unwrap().len(),
        1,
        "auto 组应在"
    );

    // 全量同步 → 移出 m1、加入 m2。
    sync_platform_manual_groups(&db, p.id, &[m2.id])
        .await
        .unwrap();
    assert_eq!(
        get_group_platforms(&db, m1.id).await.unwrap().len(),
        0,
        "m1 应被移出"
    );
    assert_eq!(
        get_group_platforms(&db, m2.id).await.unwrap().len(),
        1,
        "m2 应被加入"
    );
    assert_eq!(
