...
pub fn force_delete_group(
    ...
    async move {
        db.call_platform_traced(None, __db_caller, move |conn| {
            conn.execute(
                "UPDATE \"group\" SET deleted_at = ?1 WHERE id = ?2",
    ...
    async move {
        ...
        let current: Vec<(i64, String)> = db
            .call_platform_traced(None, __db_caller, move |conn| {
                ...
                let binds: Vec<&dyn rusqlite::types::ToSql> = group_ids
                    .iter()
        ...
        for (gid, auto_from) in &current {
            if auto_from.is_empty() && !target.contains(gid) {
                ...
                .await
                .map_err(|e| {
                    format!("sync_platform_manual_groups: remove from group {gid}: {e}")
                })?;
                db.invalidate_group_details_cache();
            }
        }
        ...
        for &gid in &target {
            ...
            if !already {
                let existing = get_group_platforms(db, gid as u64)
                    .await
                    .unwrap_or_default();
...
#[track_caller]
pub fn get_group_platforms(
    db: &Db,
    group_id: u64,
) -> impl std::future::Future<Output = Result<Vec<GroupPlatformDetail>, String>> + '_ {
    let __db_caller = std::panic::Location::caller();
    async move {
...
fn list_group_platforms_for_groups<'a>(
    ...
    Output = Result<std::collections::HashMap<u64, Vec<GroupPlatformDetail>>, String>,
> + 'a {
    let __db_caller = std::panic::Location::caller();
    async move {
        if group_ids.is_empty() {
