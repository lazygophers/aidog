...
async fn agg_backfill_idempotent() {
    ...
    insert_proxy_log_columns(
        &db,
        ProxyLogColumns::from_log(&sample_log("x1", "g1", now), false, false),
    )
    .await
    .unwrap();
    // 全量重建一次得到基线行数。
...
async fn agg_rebuild_dedups_raw_models_to_same_actual_model() {
    ...
    rebuild_stats_agg_from_logs(&db)
        .await
        .expect("rebuild must not violate UNIQUE");

    let (rows, reqs): (i64, i64) = db
...
async fn rebuild_preserves_rows_without_matching_proxy_log() {
    ...
    assert_eq!(
        req, 1,
        "无对应 proxy_log 行的旧聚合行须保留（不再清空整表）"
    );
...
async fn correct_count_tokens_removes_orphan_buckets_and_overwrites_backed_ones() {
    ...
    chat.est_cost = 1.0;
    insert_proxy_log_columns(&db, ProxyLogColumns::from_log(&chat, false, false))
        .await
        .unwrap();
    ...
    ct.platform_id = 9;
    insert_proxy_log_columns(&db, ProxyLogColumns::from_log(&ct, false, false))
        .await
        .unwrap();
    upsert_stats_agg(
        &db,
    ...
    assert!(
        correct_count_tokens_agg_once_if_needed(&db).await.unwrap(),
        "首次应执行"
    );
