...
async fn cache_rate_never_exceeds_100() {
    ...
    assert!(
        ts.cache_rate <= 100.0,
        "today cache_rate > 100: {}",
        ts.cache_rate
    );
    assert!(
        ts.cache_rate > 98.0 && ts.cache_rate < 100.0,
        "today cache_rate expected ~99: {}",
        ts.cache_rate
    );
    ...
    assert!(
        s.overview.cache_rate <= 100.0,
        "overview cache_rate > 100: {}",
        s.overview.cache_rate
    );
    // buckets 非空（防 query_stats_inner 回归致趋势图无数据）
    assert!(
        !s.buckets.is_empty(),
        "buckets empty — trend chart would not render"
    );
}
