...
async fn price_crud_and_resolve() {
    ...
    assert_eq!(gateway::db::count_model_prices(&db).await.unwrap(), 1);
    assert_eq!(gateway::db::list_model_prices(&db, 10, 0).await.unwrap().len(), 1);
    assert_eq!(gateway::db::search_model_prices(&db, "claude", 10).await.unwrap().len(), 1);

    let f = gateway::db::filtered_list_model_prices(&db, Some("cl"), Some("github"), 10, 0).await.unwrap();
    assert_eq!(f.len(), 1);
