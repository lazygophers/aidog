...
async fn query_and_batch() {
    ...
    let q = StatsQuery { start: None, end: None, granularity: Some("day".into()), group_by: None, filter_group: None, filter_model: None, filter_platform: None };
    let _ = db::query_stats(&db, &q).await.unwrap();
    let batch = db::query_stats_batch(&db, vec![q.clone(), q]).await.unwrap();
...
async fn settings_roundtrip_and_rebuild() {
    let db = test_db().await;
    let s: StatsSettings = gateway::db::get_setting(&db, "stats", "settings").await
    ...
    gateway::db::set_setting(&db, gateway::models::SetSettingInput {
        scope: "stats".into(), key: "settings".into(),
        value: serde_json::to_value(&s).unwrap(),
    }).await.unwrap();
    gateway::db::rebuild_stats_agg_from_logs(&db).await.unwrap();
}
