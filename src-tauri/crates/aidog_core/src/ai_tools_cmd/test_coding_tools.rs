...
fn coding_tools_settings_default_is_off() {
    ...
    assert!(
        !s.apply_to_claude_plugin,
        "default apply_to_claude_plugin must be false"
    );
    assert!(
        !s.skip_claude_onboarding,
        "default skip_claude_onboarding must be false"
    );
}
...
async fn ensure_default_coding_tools_settings_no_record_no_db_write() {
    ...
    // 首次：无记录
    super::ensure_default_coding_tools_settings(&db)
        .await
        .expect("ensure first run");

    // 关键断言：ensure 不落 DB 记录（功能与开关解耦）
    let rec = aidog_db::get_setting(&db, "global", "coding_tools_settings")
        .await
        .expect("query setting");
    assert!(
        rec.is_none(),
        "ensure must NOT create DB record when none existed"
    );
}
