...
async fn settings_inbox_clear() {
    ...
    let s = gateway::db::get_notification_settings(&db).await;
    ...
    assert!(gateway::db::list_notifications(&db, 10).await.unwrap().is_empty());
    assert!(gateway::db::list_notifications(&db, 100).await.unwrap().is_empty());
    gateway::db::clear_notifications(&db).await.unwrap();
}
