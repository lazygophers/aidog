...
/// xiaomi_mimo_coding 平台转换器实现
pub struct XiaomiMimoCodingConverter;

impl PlatformConverter for XiaomiMimoCodingConverter {
    fn platform_name(&self) -> &'static str {
        "xiaomi_mimo_coding"
    }

    fn parse_openai_chat(&self, _body: &[u8]) -> Result<ChatRequest, String> {
        Err("xiaomi_mimo_coding parse_openai_chat: TODO".to_string())
    }

    fn parse_openai_completions(&self, _body: &[u8]) -> Result<ChatRequest, String> {
        Err("xiaomi_mimo_coding parse_openai_completions: TODO".to_string())
    }

    fn parse_openai_responses(&self, _body: &[u8]) -> Result<ChatRequest, String> {
        Err("xiaomi_mimo_coding parse_openai_responses: TODO".to_string())
    }

    fn parse_anthropic(&self, _body: &[u8]) -> Result<ChatRequest, String> {
        Err("xiaomi_mimo_coding parse_anthropic: TODO".to_string())
    }

    fn parse_gemini(&self, _body: &[u8]) -> Result<ChatRequest, String> {
        Err("xiaomi_mimo_coding parse_gemini: TODO".to_string())
    }

    fn to_openai_chat(&self, _req: &ChatRequest) -> Result<(Value, String), String> {
        Err("xiaomi_mimo_coding to_openai_chat: TODO".to_string())
    }

    fn to_anthropic(&self, _req: &ChatRequest) -> Result<(Value, String), String> {
        Err("xiaomi_mimo_coding to_anthropic: TODO".to_string())
    }

    fn to_gemini(&self, _req: &ChatRequest) -> Result<(Value, String), String> {
        Err("xiaomi_mimo_coding to_gemini: TODO".to_string())
    }
}
