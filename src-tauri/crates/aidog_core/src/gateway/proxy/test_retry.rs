...
fn peek_immediate_done_is_empty() {
    // 任何内容前先 [DONE] → 空响应，应重试。
    assert_eq!(
        classify_stream_first("data: [DONE]\n\n", false),
...
fn peek_error_json_is_empty() {
    ...
    assert_eq!(
        classify_stream_first(text2, false),
        StreamPeek::EmptyOrError
    );
}
...
fn peek_keepalive_only_needs_more() {
    ...
    assert_eq!(
        classify_stream_first(": ping\n\n", false),
        StreamPeek::NeedMore
    );
    assert_eq!(
        classify_stream_first("event: message_start\n", false),
        StreamPeek::NeedMore
    );
...
fn resp_headers_log_json_first_value_and_format() {
    ...
    assert_eq!(
        v.get("content-type").and_then(|x| x.as_str()),
...
fn classify_429_quota_exhausted() {
    ...
    assert!(classify_429(
        "已达到 Token Plan 用量上限：请升级套餐或购买积分补充用量。 (2056)"
    ));
...
async fn transport_retryable_connect_yes_timeout_no() {
    ...
    assert!(
        e2.is_timeout() && !e2.is_connect(),
        "构造前提：请求级读超时，实际 {e2}"
    );
    assert!(
        !is_transport_retryable(&e2, std::time::Duration::from_millis(1)),
        "读超时恒不重试"
    );
    hold.abort();
}
