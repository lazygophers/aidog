...
fn feed_sse_usage_reassembles_split_chunk_boundary() {
    ...
    assert_eq!(
        agg.tokens_in.load(Relaxed),
        723,
        "跨 chunk 重组后 input 正确"
    );
    assert_eq!(
        agg.tokens_out.load(Relaxed),
        2922,
        "跨 chunk 重组后 output 正确"
    );
    assert_eq!(
        agg.tokens_cache.load(Relaxed),
        84480,
        "跨 chunk 重组后 cache 正确"
    );
}
...
async fn flush_message_stop_writes_aggregated_body() {
    ...
    assert_ne!(
        body, "",
        "message_stop 收尾后 response_body 不应为空（聚合内容未写回）"
    );
...
async fn flush_drop_writes_partial_body() {
    ...
    assert_ne!(
        body, "",
        "Drop 兜底后 response_body 不应为空（部分内容未写回）"
    );
    assert!(
        body.contains("partial"),
        "Drop 应写回已聚合的部分内容: {body}"
    );
...
async fn flush_empty_stream_finalizes_to_empty_not_placeholder() {
    ...
    for _ in 0..200 {
        let l = aidog_logs::get_proxy_log(&state.db, id)
            .await
            .unwrap()
            .unwrap();
        if l.done {
...
fn replace_model_aligns_to_requested() {
    ...
    assert!(
        String::from_utf8(out)
            .unwrap()
            .contains("\"model\":\"requested-name\"")
    );
    let aligned = br#"{"id":"x","model":"requested-name"}"#;
    assert_eq!(
        replace_model_in_json(aligned, "requested-name"),
        aligned.to_vec()
    );
}
