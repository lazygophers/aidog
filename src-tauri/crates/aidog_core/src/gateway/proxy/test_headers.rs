...
fn build_upstream_headers_passes_through_and_overrides_auth() {
    ...
    assert!(
        m.get("anthropic-beta").unwrap().contains("claude-code-"),
        "anthropic-beta overridden by simulation snapshot"
    );
    ...
    assert!(
        m.get("x-api-key").unwrap().contains("****"),
        "x-api-key must be redacted platform key"
    );
...
fn is_official_anthropic_host_variants() {
    ...
    assert!(
        is_official_anthropic_host("https://API.ANTHROPIC.COM/v1/messages"),
        "case-insensitive"
    );
    assert!(
        is_official_anthropic_host("https://api.anthropic.com:443/v1/messages"),
        "with port"
    );
    assert!(!is_official_anthropic_host(
        "https://open.bigmodel.cn/api/anthropic/v1/messages"
    ...
    assert!(!is_official_anthropic_host(
        "https://third-party.example.com/v1/messages"
    ));
    assert!(!is_official_anthropic_host(""));
}
...
fn passthrough_headers_strips_hop_by_hop() {
    ...
    assert!(
        ...
        "auth passes through in passthrough path"
    );
}
...
fn passthrough_convert_headers_strips_auth_and_ua() {
    ...
    assert!(
        ...
        "x-api-key stripped in convert path"
    );
    assert!(
        !out.contains_key("user-agent"),
        "ua stripped in convert path"
    );
    assert!(
        !out.contains_key("content-type"),
        "ct stripped in convert path"
    );
...
fn build_upstream_headers_codex_openai() {
    ...
    assert!(
        !m.contains_key("conversation_id"),
        "conversation_id must not be injected"
    );
    assert!(
        !m.contains_key("session_id"),
        "session_id must not be injected"
    );
    assert!(
        m.get("Authorization").unwrap().starts_with("Bearer "),
        "Bearer auth"
    );
}
...
fn build_upstream_headers_default_client_gemini() {
    ...
    let m: std::collections::HashMap<&str, &str> =
        h.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    assert!(
        m.contains_key("x-goog-api-key"),
        "gemini uses x-goog-api-key"
    );
    assert!(
        !m.contains_key("Authorization"),
        "no Authorization for gemini"
    );
}
...
fn build_upstream_headers_claude_code_variants() {
    ...
    for (ct, expected_part) in &variants {
        ...
        assert!(
            ua.contains(expected_part),
            "UA for {ct:?} should contain '{expected_part}', got: {ua}"
        );
    }
}
...
fn uuid_sim_format_and_uniqueness() {
    ...
    assert!(
        a.contains('-'),
        "uuid_sim output should contain hyphens: {a}"
    );
    // Should not be empty
    assert!(!a.is_empty());
    assert!(!b.is_empty());
...
fn inject_coding_plan_fields_kimi_adds_prompt_cache_key() {
    ...
    assert!(
        key.starts_with("aidog-"),
        "key should start with aidog-: {key}"
    );
}
...
fn inject_coding_plan_fields_other_protocols_no_op() {
    ...
    for proto in &protocols {
        ...
        assert!(
            ...
            "non-Kimi protocol should not inject: {proto:?}"
        );
    }
}
...
fn build_upstream_headers_claude_code_openai_third_party() {
    ...
    assert!(
        m.get("Authorization").unwrap_or(&"").starts_with("Bearer "),
        "OpenAI bearer auth: {:?}",
        m
    );
}
...
fn build_upstream_headers_codex_gemini() {
    ...
    assert!(
        !m.contains_key("OpenAI-Beta"),
        "no OpenAI-Beta for gemini protocol"
    );
}
...
fn apply_client_headers_claude_code_anthropic_third_party_strips_beta() {
    ...
    // 其余特征头照常注入
    assert!(
        h.contains_key("x-stainless-lang"),
        "non-beta feature headers still injected"
    );
    assert!(
        h.contains_key("anthropic-version"),
        "anthropic-version still injected"
    );
}
...
fn apply_client_headers_claude_code_other_protocol() {
    ...
    let rb = apply_client_headers(
        rb,
        &ct("claude_code_sdk_ts"),
        &crate::gateway::models::Protocol::DeepSeek,
        "sk-key",
    );
    let h = headers_from_builder(rb);
    assert!(h.contains_key("Authorization"));
...
fn apply_client_headers_codex_other() {
    ...
    let rb = apply_client_headers(
        rb,
        &ct("codex_desktop"),
        &crate::gateway::models::Protocol::OpenRouter,
        "sk-key",
    );
    let h = headers_from_builder(rb);
    assert!(h.contains_key("Authorization"));
...
fn apply_client_headers_cursor_other() {
    ...
    let rb = apply_client_headers(
        rb,
        &ct("cursor"),
        &crate::gateway::models::Protocol::Kimi,
        "sk-key",
    );
    let h = headers_from_builder(rb);
    assert!(h.contains_key("Authorization"));
...
fn apply_all_client_types_anthropic_protocol() {
    ...
    for &c in ALL_CLIENT_TYPES {
        ...
        assert!(
            ...
            "[{c}] anthropic: no OpenAI-Beta"
        );
        // UA
        if expect_has_ua(c) {
            let ua = h.get("User-Agent").unwrap().to_str().unwrap();
            assert!(
                ua.contains(expected_ua_substring(c).unwrap()),
                "[{c}] UA mismatch: {ua}"
            );
        } else {
...
fn apply_all_client_types_openai_protocol() {
    ...
    for &c in ALL_CLIENT_TYPES {
        ...
        assert!(
            ...
            "[{c}] openai: session_id must not be injected"
        );
        assert!(
            !h.contains_key("conversation_id"),
            "[{c}] openai: conversation_id must not be injected"
        );
        // UA
        if expect_has_ua(c) {
            let ua = h.get("User-Agent").unwrap().to_str().unwrap();
...
fn apply_all_client_types_gemini_protocol() {
    ...
    for &c in ALL_CLIENT_TYPES {
        ...
        let h = headers_from_builder(rb);
        assert!(
            h.contains_key("x-goog-api-key"),
            "[{c}] gemini: x-goog-api-key"
        );
        assert!(
            !h.contains_key("OpenAI-Beta"),
            "[{c}] gemini: no OpenAI-Beta"
        );
        assert!(
            !h.contains_key("Authorization"),
            "[{c}] gemini: no Authorization"
        );
        if expect_has_ua(c) {
            let ua = h.get("User-Agent").unwrap().to_str().unwrap();
            assert!(
...
fn apply_client_headers_unknown_client_type_fallback_bearer() {
    ...
    assert!(
        h.contains_key("Authorization"),
        "unknown client_type: Bearer fallback"
    );
    ...
    assert!(
        h.contains_key("api-key"),
        "unknown client_type: default entry → api-key present"
    );
}
...
fn apply_client_headers_unknown_client_type_anthropic_uses_default_entry() {
    ...
    let h = headers_from_builder(rb);
    assert!(
        h.contains_key("x-api-key"),
        "unknown + anthropic → x-api-key (default entry)"
    );
    assert!(
        !h.contains_key("Authorization"),
        "unknown + anthropic → no Bearer"
    );
...
fn apply_client_headers_unknown_client_type_gemini_uses_default_entry() {
    ...
    assert!(
        h.contains_key("x-goog-api-key"),
        "unknown + gemini → x-goog-api-key (default entry)"
    );
    ...
    for &c in ALL_CLIENT_TYPES {
        for (proto, proto_key) in &protocols {
            ...
            if *proto == crate::gateway::models::Protocol::Anthropic {
                ...
                assert!(
                    v.contains("****"),
                    "[{c}/{proto_key}] Authorization redacted: {v}"
                );
            }
            // codex×openai extras
            assert_eq!(
