...
fn count_tokens_endpoint_detection() {
    ...
    assert!(is_count_tokens_endpoint(
        "/glm-coding-plan-auto/v1/messages/count_tokens"
    ));
...
fn per_model_claude_equals_gpt4_cl100k() {
    ...
    assert_eq!(
        claude, gpt4,
        "claude-* and gpt-4 both route to cl100k, must be equal"
    );
...
fn estimate_input_tokens_per_model_routing() {
    ...
    assert_ne!(
        gpt4o, gpt4,
        "gpt-4o (o200k) should diverge from gpt-4 (cl100k)"
    );
}
...
fn estimate_input_tokens_empty_floor() {
    use super::estimate_input_tokens;
    assert_eq!(
        estimate_input_tokens(&serde_json::json!({}), "claude-3-opus"),
        1
    );
    assert_eq!(
        estimate_input_tokens(&serde_json::Value::Null, "claude-3-opus"),
        1
    );
...
fn estimate_input_tokens_recursive_collect() {
    ...
    assert!(
        n > 2,
        "multi-segment content should accumulate both texts, got {n}"
    );
}
