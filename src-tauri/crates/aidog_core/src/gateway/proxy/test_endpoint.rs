...
fn same_protocol_passthrough_condition() {
    ...
    assert!(
        !pass,
        "openai_responses→openai fallback must NOT passthrough (needs conversion)"
    );
...
fn select_endpoint_coding_plan_exclusivity() {
    ...
    let m = sel(&kimi_cp, &Protocol::Anthropic)
        .expect("anthropic inbound must resolve to coding endpoint");
    assert_eq!(m.base_url, "https://api.kimi.com/coding/v1");
    assert!(
        m.coding_plan,
        "selected endpoint must be the coding endpoint"
    );
...
fn endpoint_host_extraction() {
    ...
    assert_eq!(
        host("https://open.bigmodel.cn/api/anthropic").as_deref(),
        Some("open.bigmodel.cn")
    );
    assert_eq!(
        host("https://open.bigmodel.cn/api/coding/paas/v4").as_deref(),
        Some("open.bigmodel.cn")
    );
    // 端口被剥离
    assert_eq!(
        host("http://localhost:8080/v1").as_deref(),
        Some("localhost")
    );
    // 大小写归一
    assert_eq!(
        host("https://API.Kimi.COM/coding/v1").as_deref(),
        Some("api.kimi.com")
    );
    // userinfo 被剥离
    assert_eq!(
        host("https://user:pass@api.moonshot.cn/anthropic").as_deref(),
        Some("api.moonshot.cn")
    );
    // 无 scheme 也能取 host
    assert_eq!(
        host("api.moonshot.cn/anthropic").as_deref(),
        Some("api.moonshot.cn")
    );
...
fn infer_passthrough_protocol_from_ua_mapping() {
    ...
    assert_eq!(
        infer("claude-cli/1.0.117 (external, cli)"),
        Some(Protocol::Anthropic)
    );
    assert_eq!(
        infer("claude-cli/1.0.117 (external, claude-vscode, agent-sdk/0.1.30)"),
        Some(Protocol::Anthropic)
    );
    // Codex 家族（codex_cli_rs / Codex/ / codex desktop / codex-vscode；大小写不敏感）
    assert_eq!(
        infer("codex_cli_rs/0.38.0 (MacOS; arm64) Terminal"),
        Some(Protocol::OpenAIResponses)
    );
    assert_eq!(infer("Codex/0.38.0"), Some(Protocol::OpenAIResponses));
    assert_eq!(
        infer("codex desktop/0.38.0"),
        Some(Protocol::OpenAIResponses)
    );
    assert_eq!(
        infer("codex-vscode/0.38.0"),
        Some(Protocol::OpenAIResponses)
    );
...
fn ua_passthrough_three_level_fallback() {
    ...
    // 模拟平台端点协议集合
    let try_passthrough =
        |ua: &str, platform_protos: &[Protocol], source_matched: bool| -> Option<Protocol> {
...
fn select_endpoint_empty_endpoints_fallback_to_invalid_platform_type() {
    ...
    assert!(
        !is_valid(&sensenova_type),
        "SenseNova must not be a valid wire protocol"
    );
...
fn select_endpoint_cross_protocol_fallback() {
    ...
    assert!(
        m.coding_plan,
        "selected endpoint must be coding (401 protection)"
    );
    ...
    let empty: Vec<PlatformEndpoint> = vec![];
    assert!(
        sel(&empty, &Protocol::Anthropic).is_none(),
        "empty endpoints must return None"
    );
    assert!(sel(&empty, &Protocol::OpenAI).is_none());
    assert!(sel(&empty, &Protocol::Gemini).is_none());
}
...
fn detect_source_protocol_bare_endpoint_without_v1() {
    ...
    assert_eq!(
        d("/v1beta/models/gemini-pro:generateContent"),
        Protocol::Gemini
    );
...
fn model_from_gemini_path_extracts_model_segment() {
    ...
    assert_eq!(
        m("/v1beta/models/gemini-2.5-pro:generateContent"),
        "gemini-2.5-pro"
    );
    assert_eq!(
        m("/v1beta/models/gemini-2.5-pro:streamGenerateContent"),
        "gemini-2.5-pro"
    );
    assert_eq!(
        m("/proxy/v1beta/models/gemini-1.5-flash:countTokens"),
        "gemini-1.5-flash"
    );
    // 无方法段 / 非 gemini 路径
