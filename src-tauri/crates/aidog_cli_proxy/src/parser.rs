...
/// 解析任一 CPA provider 数组段为 `CpaProvider` 列表。
///
/// 6 段 (gemini/interactions/codex/claude/openai-compat/vertex) 共享同构提取：
/// `base-url` / `models` / `prefix` / `headers` 一致，仅 api_key 来源 + name/disabled 不同。
///
    ...
    for item in arr {
        ...
        let (api_key, name, disabled) = if openai_compat {
            ...
            let api_key = obj
                ...
                .unwrap_or("");
            (api_key.to_string(), name, disabled)
        } else {
            let api_key = obj.get("api-key").and_then(|v| v.as_str()).unwrap_or("");
            (api_key.to_string(), None, false)
        };
        if api_key.is_empty() {
            continue;
        }
...
// ─── 压缩解压 (移至 archive.rs) ───────────────────────────────────────
// unzip_archive / untar_archive / is_supported_archive / is_unsupported_archive
// 见 super::archive
