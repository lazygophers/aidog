...
pub fn to_gemini(req: &ChatRequest) -> GeminiRequest {
    ...
    let generation_config = if req.max_tokens.is_some() || req.temperature.is_some() || req.top_p.is_some()
        ...
        Some(GeminiGenerationConfig {
            ...
            response_mime_type,
            response_schema,
            // includeThoughts 挂在 thinkingConfig 上，故随 thinkingBudget 一起写出
            // （thinkingBudget 的取值与类型归票 03，本票只挂载新字段）
            thinking_config: req.thinking_budget.map(|b| GeminiThinkingConfig { thinking_budget: b, include_thoughts }),
        })
    } else {
        None
    };
