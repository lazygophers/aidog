...
impl ProtocolConverter for MockConverter {
    ...
    fn parse_incoming(&self, body: &[u8]) -> Result<ChatRequest, String> {
        serde_json::from_slice(body)
            .map_err(|e| format!("Mock parse error: {}", e))
    }
    ...
    fn parse_response(&self, body: &[u8]) -> Result<NonStreamResponse, String> {
        ...
        Ok(NonStreamResponse {
            ...
            text: value.get("content").and_then(|v| v.as_str()).map(String::from),
            tool_uses: Vec::new(),
            stop_reason: value.get("finish_reason").and_then(|v| v.as_str()).unwrap_or("end_turn").to_string(),
            input_tokens: 0,
            output_tokens: 0,
            cache_read_tokens: 0,
            reasoning: None,
