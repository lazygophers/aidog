...
async fn backup_settings_get_default() {
    ...
    let settings = crate::BackupSettings::load(&db).await.sanitized();
...
async fn backup_settings_set_and_get_roundtrip() {
    ...
    let mut settings = crate::BackupSettings::load(&db).await;
    ...
    let got = crate::BackupSettings::load(&db).await.sanitized();
    assert!(got.enabled);
...
async fn backup_run_now_returns_result() {
    ...
    let result = crate::run_backup(&db).await;
    let _ = result;
}
