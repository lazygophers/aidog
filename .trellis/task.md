...
| model-info-source | 模型信息中心化: 移除旧价格同步, 改 GitHub JSON 为唯一信源 (price/max_tokens/context) | — | 规划中 | plan | 0% | — |
| pricing-python | Python 定价工程 + data/models.json 首版 (scripts/pricing/ uv + 11 scraper + Makefile 单入口) | — | 规划中 | 规划 | 0% | — |
| pricing-rust-sync | Rust: migration 008 max_*列 + 同步源换 GitHub raw + db max_* + 移除旧 price_sync/upsert/delete + import_export 移除 model_price scope | — | 规划中 | 规划 | 0% | — |
| pricing-frontend | 前端 PricingTab 改造: 移除手动编辑/删除 + 加 max_tokens/context 列 + 文案 GitHub + api.ts + i18n 7 语言 | — | 规划中 | 规划 | 0% | — |
| max-tokens-cap | max_tokens 入站 parse(anthropic/gemini) + router 转换前裁剪(仅超上限裁, 未传不注入) | — | 规划中 | 规划 | 0% | — |
