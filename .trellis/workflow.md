...
📌 **等 notification 回调时输出任务清单 (finish 限定)**: 当 workflow 异步跑完成后等 `<task-notification>` 回调 (finish 前等回调) 时, AI 每轮回传 MUST 输出 4 列任务清单表格 (同 in_progress 段异步等待清单, 状态本地化), 列结构:

| id | 状态 | 摘要 | 进度% |
| --- | --- | --- | --- |

仅此「等 notification」语境触发; finish 动作本身 (跑 `task.py finish`) 不输出。格式主源 = trellisx-orchestrate `progress-communication.md` §异步等待清单格式。
