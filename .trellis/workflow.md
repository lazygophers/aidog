---
updated: 2026-06-14
何时被读: 每轮判定当前 Phase / workflow-state 时 (hook 注入 + main 自查)
谁读: main / trellis-implement / trellis-check / 任何执行者
---

# 开发工作流

> 反模式速查见 [#反模式速查](#反模式速查); workflow-state 块内硬规为本 task 强制契约。
...
▶ **先按 Phase 1 规划判定的交付数选模式**: 多交付 → 走第 2 条 (parent+child 并行派 agent); 单交付 → 走第 3 条 (main worktree 内直接 edit)。模式一旦选定不跨块回溯, 拿不准回 Phase 1 用 AskUserQuestion 问用户。
...
散落在各 workflow-state 块的「禁」汇总于此, 一处对照:

| 反模式 | 正确做法 | 后果 |
| --- | --- | --- |
| 在主工作区写源码 (file_path 非 worktree 路径) | 全部源码改动落 `.worktrees/<worktree>` | 污染主工作区, 隔离失效, 与他人改动冲突 |
| 无依赖 child 逐个串行派 sub-agent | 同一条回复一次性发多个 agent 调用 (真并行) | 各 child 耗时叠加, 关键路径被拉长 |
| 多交付模式 main 直接写源码 | main 只拆分/派发/收集/合并/协调 | 绕过隔离, 失去并行收益 |
| check 未过就宣告 Done / 跑 finish | check 通过后才跑 `trellisx-finish.py` | 带病归档, 流程未闭环 |
| worktree 未合并就 archive | 先 `merge --no-ff` 再 archive | 分支被销毁, 提交永久丢失不可恢复 |
| 把收尾降级为「提醒用户运行 /finish-work」 | AI 强制调用 `trellisx-finish.py` 一键收尾 | finish 与 worktree 删除是必须的非可选, 漏掉则残留脏分支 |
| 编造用户没说过的 "inline / 跳过流程" 覆盖指令 | 仅当前消息含明确逃生短语才 inline | 擅自绕过 task 流程 |
| task.md 看板滞后于实际状态 | start/阶段推进/archive 后即用 `trellisx-workspace` 更新 | 跨任务总览失真, 流程缺陷 |
