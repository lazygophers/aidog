...
| [HTML JSON Embedding](./html-json-embedding.md) | server-side 模板把 JSON 嵌入 `<script type="application/json">` 时 | `JSON.parse(textContent)` 运行时炸 SyntaxError |
...
- server-side 生成 HTML 嵌入 JSON (`<script type="application/json">`) → [HTML JSON Embedding](./html-json-embedding.md) (禁 html.escape，仅 replace `<`→`<`)
