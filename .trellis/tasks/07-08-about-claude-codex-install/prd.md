...
## Decision (ADR-lite, AFK best-judgment 2026-07-08, 用户回来可推翻)

**Context**: grill 硬门1 5 决策点，用户 AFK 超时，按 prd 倾向 + research 建议推断。
**Decision**:
1. MVP 工具范围：**claude + codex only**（research 建议，降维护成本；gemini/opencode/hermes 后续迭代）
2. 版本检查触发：**仅手动按钮**（无后台定时，纯手动控；同 prd out of scope「不实现定时后台升级」）
3. 安装/升级执行：**后端直接 spawn**（抄 install_uv 模式，toast 反馈结果；装信任用户）
4. 自动升级：**默认关**（纯手动，无定时检查）
5. 冲突修复：**只报告 + 建议**（不自动卸载，同 prd out of scope「不自动卸载冲突二进制」）
**Consequences**: MVP scope 收敛 claude+codex；交互纯手动；破坏性操作（卸载）不自动执行。
