...
每条规则都同时写了 `Write(...)` 和 `Edit(...)`：这在权限系统里是两个独立工具，只拦一个等于没拦。

### ask — 依赖清单需确认

`go.mod`、`package.json`、`Cargo.toml`、`Gemfile`、`pyproject.toml`、`requirements.txt`、`Pipfile`、`deno.json`、`bunfig.toml`、`composer.json`、`Podfile`、`mix.exs` 同样是 `Write` + `Edit` 成对；另有构建与容器清单（`Makefile`、`Dockerfile`、`CMakeLists.txt`、`pom.xml`、`build.gradle`、`Chart.yaml`、`*.csproj`、`turbo.json` 等）只拦 `Write`。
