...
/** StatusLine template definitions */
interface StatusLineTemplate {
  id: string;
  name: string;
  desc: string;
  /** Recommended refreshInterval, 0 = event-only */
  refreshInterval: number;
  generate: (opts: TemplateOptions) => string;
}
...
const DEFAULT_TEMPLATE_OPTS: TemplateOptions = {
  showModel: true,
  showContext: true,
  showGit: true,
  showCost: false,
...
// ── Template script generators (bash + jq) ──

const SCRIPT_HEADER = `#!/usr/bin/env bash
# Generated by aidog — do not edit manually
input=$(cat)`;

function ctxBar(opts: TemplateOptions): string {
  const parts: string[] = [];
  if (opts.showModel) {
    parts.push(`model=$(echo "$input" | jq -r '.model.display_name // "Claude"')`);
    parts.push(`echo -n "[$model] "`);
  }
  parts.push(`pct=$(echo "$input" | jq -r '(.context_window.used_percentage // 0) | round')`);
  parts.push(`filled=$((pct / 10))`);
  parts.push(`empty=$((10 - filled))`);
  parts.push(`bar=""`);
  parts.push(`for ((i=0; i<filled; i++)); do bar+="▓"; done`);
  parts.push(`for ((i=0; i<empty; i++)); do bar+="░"; done`);
  parts.push(`echo "$bar $pct%"`);
  return [SCRIPT_HEADER, ...parts].join("\n");
}

function gitStatus(opts: TemplateOptions): string {
  const parts: string[] = [];
  parts.push(`branch=$(echo "$input" | jq -r '.workspace.repo.name // "detached"')`);
  if (opts.showModel) {
    parts.push(`model=$(echo "$input" | jq -r '.model.display_name // ""')`);
    parts.push(`echo -n "$branch · $model"`);
  } else {
    parts.push(`echo -n "$branch"`);
  }
  return [SCRIPT_HEADER, ...parts].join("\n");
}

function costTracker(_opts: TemplateOptions): string {
...
m=$((s / 60))
s=$((s % 60))
printf "\\$%.2f · %dm %ds" "$cost" "$m" "$s"`;
}

function multiLine(opts: TemplateOptions): string {
  const lines: string[] = [SCRIPT_HEADER];
  const line1Parts: string[] = [];
  const line2Parts: string[] = [];

  if (opts.showGit) {
    line1Parts.push('$(echo "$input" | jq -r \'.workspace.repo.name // "detached"\')');
  }
  if (opts.showModel) {
    line1Parts.push('$(echo "$input" | jq -r \'.model.display_name // "Claude"\')');
  }
  if (opts.showEffort) {
    line1Parts.push('$(echo "$input" | jq -r \'(.effort.level // "")\')');
  }

  if (opts.showContext) {
    line2Parts.push('$(echo "$input" | jq -r \'(.context_window.used_percentage // 0) | round\')%');
  }
  if (opts.showCost) {
    line2Parts.push('$(echo "$input" | jq -r \'(.cost.total_cost_usd // 0) * 100 | round / 100\' | xargs printf "\\$%.2f")');
  }
  if (opts.showRateLimits) {
    line2Parts.push('5h:$(echo "$input" | jq -r \'(.rate_limits.five_hour.used_percentage // "?")\')%');
  }

  if (line1Parts.length > 0) {
    lines.push(`echo "${line1Parts.join(" · ")}"`);
  }
  if (line2Parts.length > 0) {
    lines.push(`echo "${line2Parts.join(" · ")}"`);
  }
  if (line1Parts.length === 0 && line2Parts.length === 0) {
    lines.push(`echo "$input" | jq -r '.model.display_name // "Claude"'`);
  }
  return lines.join("\n");
}

function minimal(_opts: TemplateOptions): string {
  return `${SCRIPT_HEADER}
...
const STATUSLINE_TEMPLATES: StatusLineTemplate[] = [
  { id: "multi-line", name: "多行综合", desc: "Git + 模型 + 进度条 + 成本", refreshInterval: 5, generate: multiLine },
  { id: "context-bar", name: "上下文进度条", desc: "模型名 + 10字符进度条 + 百分比", refreshInterval: 0, generate: ctxBar },
  { id: "git-status", name: "Git 状态", desc: "分支名 + 模型", refreshInterval: 0, generate: gitStatus },
  { id: "cost-tracker", name: "成本追踪", desc: "API 成本 + 持续时间", refreshInterval: 5, generate: costTracker },
  { id: "minimal", name: "极简", desc: "仅模型名 + 上下文%", refreshInterval: 0, generate: minimal },
  { id: "custom", name: "自定义", desc: "自由组合字段", refreshInterval: 5, generate: multiLine },
];
...
const SUBAGENT_TEMPLATES: SubagentTemplate[] = [
  {
    id: "default",
    name: "默认",
    desc: "任务名 + 状态 + token 数",
    generate: () => `#!/usr/bin/env bash
# Generated by aidog — subagent statusline
input=$(cat)
...
const STATUSLINE_DATA_FIELDS = [
  ...
  { group: "成本", fields: [
    ...
    { key: "cost.total_lines_added", desc: "新增代码行数" },
    { key: "cost.total_lines_removed", desc: "删除代码行数" },
  ]},
  { group: "上下文窗口", fields: [
    { key: "context_window.used_percentage", desc: "已使用百分比" },
    { key: "context_window.context_window_size", desc: "最大窗口大小" },
    { key: "context_window.total_input_tokens", desc: "当前输入 tokens" },
    { key: "context_window.total_output_tokens", desc: "当前输出 tokens" },
  ]},
  { group: "速率限制", fields: [
  ...
  { group: "其他", fields: [
    { key: "effort.level", desc: "推理工作量 (low/medium/high/xhigh/max)" },
    { key: "vim.mode", desc: "Vim 模式 (NORMAL/INSERT/VISUAL)" },
    { key: "session_id", desc: "会话 ID" },
    { key: "version", desc: "Claude Code 版本" },
    { key: "pr.number / pr.url", desc: "当前分支 PR" },
    { key: "worktree.name", desc: "活跃 worktree 名称" },
  ]},
];
...
function StatusLinePanel({
  ...
  const templateId = stored.template ?? (isMain ? "multi-line" : "default");
  const padding = stored.padding ?? 2;
  const refreshInterval = stored.refreshInterval ?? 5;
  const hideVimModeIndicator = !!stored.hideVimModeIndicator;
  const opts = { ...DEFAULT_TEMPLATE_OPTS, ...(stored.options ?? {}) };

  const templates = isMain ? STATUSLINE_TEMPLATES : SUBAGENT_TEMPLATES;
  const currentTemplate = templates.find(tp => tp.id === templateId) ?? templates[0];

  // Generate script preview
  const scriptPreview = isMain
    ? currentTemplate.generate(opts)
    : (currentTemplate as SubagentTemplate).generate();
  ...
  const setStored = (patch: Record<string, any>) => {
    ...
    updateField(aidogKey, next);
  };

  const handleToggle = (val: boolean) => {
    if (val) {
      // Enable: generate script + write statusLine config
      setStored({ ...stored, enabled: true });
    } else {
      // Disable: remove statusLine from config, keep _aidog_ settings
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const path = await statuslineApi.generate(scriptType, script);
      const statusLineValue: Record<string, any> = {
        type: "command",
        command: path,
      };
      if (isMain) {
        if (padding > 0) statusLineValue.padding = padding;
        if (refreshInterval > 0) statusLineValue.refreshInterval = refreshInterval;
        if (hideVimModeIndicator) statusLineValue.hideVimModeIndicator = true;
      }
      updateField(fieldName, statusLineValue);
    } catch (e: any) {
  ...
  const showFieldOpts = isMain && (templateId === "multi-line" || templateId === "custom");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: S.gap }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <Hint>
            {isMain
              ? t("statusline.hint", "开启后 aidog 生成脚本到 ~/.aidog/aidog-statusline.sh")
              : t("statusline.subagentHint", "开启后 aidog 生成脚本到 ~/.aidog/aidog-subagent-statusline.sh")}
          </Hint>
        </div>
        {enabled && (
          <span style={{
      ...
      {enabled && (
        <>
          ...
          <div style={{
            ...
            <SubHeading>
              <SvgIcon d={ICON_PATHS.status} size={15} />
              {t("statusline.template", "模板")}
            </SubHeading>
            ...
            <Hint>{currentTemplate.desc}</Hint>
          </div>

          {/* Field options for multi-line / custom */}
          {showFieldOpts && (
            <div style={{
              padding: "14px 16px", background: "var(--bg-glass)", borderRadius: "var(--radius-md)",
              display: "flex", flexDirection: "column", gap: 10,
              ...
              <SubHeading>
                <SvgIcon d={ICON_PATHS.ui} size={15} />
                {t("statusline.fields", "显示字段")}
              </SubHeading>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {([
                  ...
                  ["showCost", "成本追踪"],
                  ["showRateLimits", "速率限制"],
                  ["showEffort", "Effort level"],
                  ["showVim", "Vim 模式"],
                ] as const).map(([key, label]) => (
                  <label key={key} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: F.body, color: "var(--text-primary)", cursor: "pointer" }}>
                    <Toggle active={!!opts[key as keyof TemplateOptions]} onChange={(v) => {
                      setStored({ options: { ...opts, [key]: v } });
                    }} />
                    {label}
                  </label>
                ))}
          ...
          {/* Options row (main statusline only) */}
          {isMain && (
            <div style={{
              padding: "14px 16px", background: "var(--bg-glass)", borderRadius: "var(--radius-md)",
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <label style={{ fontSize: F.body, color: "var(--text-secondary)", whiteSpace: "nowrap" }}>
                  {t("statusline.padding", "水平间距")}
                </label>
                <input className="input" type="number" min={0} max={20}
                  style={{ width: 70, fontSize: F.body, padding: S.inputPad }}
                  value={padding}
                  onChange={(e) => setStored({ padding: Math.max(0, Number(e.target.value)) })} />
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <label style={{ fontSize: F.body, color: "var(--text-secondary)", whiteSpace: "nowrap" }}>
                  {t("statusline.refreshInterval", "刷新间隔")}
                </label>
                <input className="input" type="number" min={0} max={300}
                  style={{ width: 70, fontSize: F.body, padding: S.inputPad }}
                  value={refreshInterval}
                  onChange={(e) => setStored({ refreshInterval: Math.max(0, Number(e.target.value)) })} />
                <span style={{ fontSize: F.hint, color: "var(--text-tertiary)" }}>
                  {t("statusline.seconds", "秒 (0=仅事件)")}
                </span>
              </div>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: F.body, color: "var(--text-secondary)", cursor: "pointer" }}>
                <Toggle active={hideVimModeIndicator}
                  onChange={(v) => setStored({ hideVimModeIndicator: v })} />
                {t("statusline.hideVimMode", "隐藏 Vim 模式指示器")}
              </label>
            </div>
          )}
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <button type="button" className="btn btn-ghost"
                style={{ fontSize: F.body, padding: "4px 8px", display: "flex", alignItems: "center", gap: 4 }}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            <button className="btn btn-primary"
              ...
              {saving ? t("status.generating", "生成中…") : t("statusline.apply", "应用并生成脚本")}
            </button>
          </div>
        </>
      )}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: S.gap }}>
      ...
      <div style={{
        ...
        <StatusLinePanel config={config} updateField={updateField} t={t} scriptType="statusline" />
      </div>
      ...
      <div style={{
        ...
        <StatusLinePanel config={config} updateField={updateField} t={t} scriptType="subagent" />
      </div>
      ...
      <div style={{
        ...
        <button type="button" className="btn btn-ghost"
          ...
          {t("statusline.dataRef", "可用数据字段参考")}
        </button>
        {showDataRef && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 12 }}>
            <Hint>{t("statusline.dataRefHint", "Claude Code 通过 stdin 注入以下 JSON 字段，可在脚本中用 jq 提取")}</Hint>
