...
import type { SharePlatform } from "../../services/api";

export type ShareFormat = "yaml" | "json" | "base64";

export interface ShareModalProps {
  /** 后端 platform_share_export 返回的可分享配置对象（含明文 api_key）。 */
  share: SharePlatform;
  /** 分享平台显示名（弹窗标题用）。 */
  platformName: string;
...
function formatShare(share: SharePlatform, fmt: ShareFormat): string {
...
export function ShareModal({ share, platformName, onToast, onClose }: ShareModalProps) {
    ...
    >
      ...
      >
        <div style={{ fontSize: 17, fontWeight: 600, color: "var(--text-primary)", marginBottom: 6 }}>
          {t("platform.share.title", "分享平台")} · {platformName}
        </div>
